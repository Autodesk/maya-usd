...
StubCommandHook::StubCommandHook(SessionState* sessionState)
    : AbstractCommandHook(sessionState)
{
}

void StubCommandHook::setEditTarget(UsdLayer layer)
{
    _calls.push_back({ "setEditTarget", { layer->GetIdentifier() } });
}

void StubCommandHook::insertSubLayerPath(UsdLayer layer, Path path, int index)
{
    _calls.push_back(
...
void StubCommandHook::removeSubLayerPath(UsdLayer layer, Path path)
{
...
void StubCommandHook::replaceSubLayerPath(UsdLayer layer, Path oldPath, Path newPath)
{
...
void StubCommandHook::discardEdits(UsdLayer layer)
{
    _calls.push_back({ "discardEdits", { layer->GetIdentifier() } });
    layer->Clear();
}

void StubCommandHook::clearLayer(UsdLayer layer)
{
    _calls.push_back({ "clearLayer", { layer->GetIdentifier() } });
    layer->Clear();
}

void StubCommandHook::flattenLayer(UsdLayer layer)
{
...
void StubCommandHook::muteSubLayer(UsdLayer layer, bool muteIt)
{
...
void StubCommandHook::stitchLayers(const std::vector<PXR_NS::SdfLayerRefPtr>& /*layers*/)
{
