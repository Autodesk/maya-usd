...
TEST(SplitString, basicTest)
{
    ...
    std::vector<std::string> tokens = MayaUsd::ufe::splitString(testString, ", ?");
    ...
    std::vector<std::string> tokens2 = MayaUsd::ufe::splitString(testString, "!");
    ASSERT_EQ(static_cast<int>(tokens2.size()), 1);
    ASSERT_STREQ(tokens2[0].c_str(), testString.c_str());

    std::vector<std::string> tokens3 = MayaUsd::ufe::splitString(testString, "");
