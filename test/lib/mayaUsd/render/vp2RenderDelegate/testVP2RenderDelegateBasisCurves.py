...
class testVP2RenderDelegateBasisCurves(imageUtils.ImageDiffingTestCase):
    ...
    def _RunTest(self, complexity, selection):
        ...
        self.assertSnapshotClose('%s_unselected_complexity%d%s.png' % (self._testName, complexity, self._suffix))
        globalSelection.replaceWith(selection)
        self.assertSnapshotClose('%s_selected_complexity%d%s.png' % (self._testName, complexity, self._suffix))
        globalSelection.clear()
