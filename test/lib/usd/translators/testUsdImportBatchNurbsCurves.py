...
class testUsdExportBatchCurves(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        filePath = os.path.join(inputPath, "UsdImportBasisCurvesTest", "batchNurbs.usda")
    ...
    def testExport(self):
        ...
        usdFile = os.path.abspath('UsdExportBatchNurbsTest.usda')
        cmds.usdExport(mergeTransformAndShape=True, file=usdFile,
            shadingMode='none')

        stage = Usd.Stage.Open(usdFile)
        nc = UsdGeom.NurbsCurves.Get(stage, '/NurbsBatch/NurbsBatchShape')
        ...
        stage = Usd.Stage.Open(usdFile)
        nc = UsdGeom.NurbsCurves.Get(stage, '/NurbsBatch/NurbsBatchShape1')
        self.assertEqual(nc.GetWidthsAttr().Get(), Vt.FloatArray([1.0]))
        self.assertEqual(nc.GetWidthsInterpolation(), UsdGeom.Tokens.constant)
