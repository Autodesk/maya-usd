...
class MayaUsdSchemaCommandTestCase(unittest.TestCase):
    ...
    def testApplySchemaCommand(self):
        '''Test the -schema and -appliedSchemas flags for a single-apply schema'''
        ...
        cmds.mayaUsdSchema(primUfePath, schema='MaterialBindingAPI')
        applied = cmds.mayaUsdSchema(primUfePath, appliedSchemas=True)
    ...
    def testApplySchemaToMultiplePrimsCommand(self):
        ...
        cmds.mayaUsdSchema(primUfePaths, schema='MaterialBindingAPI')
        applied = cmds.mayaUsdSchema(primUfePaths, appliedSchemas=True)
        self.assertTrue(applied)
        self.assertEqual(len(applied), 1)
        self.assertIn('MaterialBindingAPI', applied)
    ...
    def testApplyMultiSchemaCommand(self):
        ...
        self.assertRaises(RuntimeError, lambda: cmds.mayaUsdSchema(primUfePath, schema='CollectionAPI'))
        applied = cmds.mayaUsdSchema(primUfePath, appliedSchemas=True)
        self.assertFalse(applied)

        # Add a multi-apply schema and verify it has been applied.
        cmds.mayaUsdSchema(primUfePath, schema='CollectionAPI', instanceName='this')
        applied = cmds.mayaUsdSchema(primUfePath, appliedSchemas=True)
        self.assertTrue(applied)
        self.assertEqual(len(applied), 1)
