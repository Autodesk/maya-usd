...
class MayaUsdSchemaCommandTestCase(unittest.TestCase):
    ...
    def testApplySchemaToMultiplePrimsCommand(self):
        ...
        # Note: on older Maya versions, on Linux, when a command has multi-use flags,
        #        there are bugs when those flags are called from Python, so on those
        #        platforms, call the command fro MEL instead.
        if int(cmds.about(apiVersion=True)) < 20240000 and sys.platform == 'linux':
            def schemaCommand(primUfePath, appliedSchemas=False, schema=''):
                cmd = '''mayaUsdSchema'''
                if primUfePath:
                    for path in primUfePath:
                        cmd += ''' -ufe "''' + path +'''"'''
                if schema:
                    cmd += ''' -schema "''' + schema +'''"'''
                if appliedSchemas:
            ...
            schemaCommand = cmds.mayaUsdSchema

        # Verify the prims have no schema when created.
        applied = schemaCommand(primUfePath=primUfePaths, appliedSchemas=True)
        self.assertFalse(applied)

        # Add a schema to both prims and verify it has been applied.
        schemaCommand(primUfePath=primUfePaths, schema='MaterialBindingAPI')
        applied = schemaCommand(primUfePath=primUfePaths, appliedSchemas=True)
        self.assertTrue(applied)
        self.assertEqual(len(applied), 1)
        self.assertIn('MaterialBindingAPI', applied)
