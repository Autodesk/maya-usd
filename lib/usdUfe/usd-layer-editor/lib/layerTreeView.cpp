    ...
    : QTreeView(in_parent)
{
    ...
    // custom row drawing
    setCustomLayerItemDelegate(new LayerTreeItemDelegate(this));
...
{
    if (_delegate && _delegate->parent() == this) {
...
void LayerTreeView::paintEvent(QPaintEvent* event)
{
    ...
    if (model()->rowCount() == 0) {
        ...
        auto NO_LAYERS_IMAGE = getQtUtils()->createPixmap(":/RS_no_layer.png");
