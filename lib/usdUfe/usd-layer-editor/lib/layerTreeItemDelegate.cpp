    ...
    , _treeView(in_parent)
{
    ...
    if (utils->lightTheme()) {
        ...
        DISABLED_HIGHLIGHT_IMAGE = utils->lightPixmap(DISABLED_HIGHLIGHT_IMAGE, 1.2f);
        ARROW_COLOR = QColor(
            1.0f - ARROW_COLOR.redF(), 1.0f - ARROW_COLOR.greenF(), 1.0f - ARROW_COLOR.blueF());
    }

    WARNING_IMAGE = utils->createPNGResPixmap(":/UsdLayerEditor/RS_warning");
