...
{
    // TfDebug::Enable(USDUFE_UNDOSTATEDELEGATE);
}
    ...
    const SdfDataRefPtr& deletedData)
{
    ...
    TF_DEBUG(USDUFE_UNDOSTATEDELEGATE).Msg("Inverting deleting spec at '%s'\n", path.GetText());
...
void UsdUndoStateDelegate::_OnCreateSpec(const SdfPath& path, SdfSpecType specType, bool inert)
{
    ...
    if (!_setMessageAlreadyShowed) {
        TF_DEBUG(USDUFE_UNDOSTATEDELEGATE).Msg("Creating spec at '%s'\n", path.GetText());
    }
...
void UsdUndoStateDelegate::_OnDeleteSpec(const SdfPath& path, bool inert)
{
    ...
    if (!_setMessageAlreadyShowed) {
        TF_DEBUG(USDUFE_UNDOSTATEDELEGATE).Msg("Deleting spec at '%s'\n", path.GetText());
    }
    ...
    UsdUfe::UsdUndoManagerAccessor::addInverse(std::bind(
