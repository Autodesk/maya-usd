...
//----------------------------------------------------------------------------------------------------------------------
static void floatToDouble(double* output, const float* const input, size_t count)
{
...
//----------------------------------------------------------------------------------------------------------------------
static void doubleToFloat(float* output, const double* const input, size_t count)
{
  for(size_t i = 0; i < count; ++i)
  {
...
//----------------------------------------------------------------------------------------------------------------------
static void gatherFaceConnectsAndVertices(const UsdGeomMesh& mesh, MFloatPointArray& points,
...
//----------------------------------------------------------------------------------------------------------------------
static void applyHoleFaces(const UsdGeomMesh& mesh, MFnMesh& fnMesh)
{
  // Set Holes
...
//----------------------------------------------------------------------------------------------------------------------
static void applyAnimalColourSets(const UsdPrim& from, MFnMesh& fnMesh, const MIntArray& counts)
{
...
//----------------------------------------------------------------------------------------------------------------------
static bool applyVertexCreases(const UsdGeomMesh& from, MFnMesh& fnMesh)
{
...
//----------------------------------------------------------------------------------------------------------------------
static void applyAnimalVertexCreases(const UsdPrim& from, MFnMesh& fnMesh)
{
...
//----------------------------------------------------------------------------------------------------------------------
static bool applyEdgeCreases(const UsdGeomMesh& from, MFnMesh& fnMesh)
{
...
//----------------------------------------------------------------------------------------------------------------------
static void applyAnimalEdgeCreases(const UsdPrim& from, MFnMesh& fnMesh)
{
...
//----------------------------------------------------------------------------------------------------------------------
static void applyGlimpseSubdivParams(const UsdPrim& from, MFnMesh& fnMesh)
{
...
//----------------------------------------------------------------------------------------------------------------------
static void copyFaceConnectsAndPolyCounts(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
...
static void copyColourSetData(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  UsdPrim prim = mesh.GetPrim();
...
//----------------------------------------------------------------------------------------------------------------------
static void copyInvisibleHoles(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
...
//----------------------------------------------------------------------------------------------------------------------
static void copyGlimpseTesselationAttributes(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  MStatus status;
  MPlug plug;
...
//----------------------------------------------------------------------------------------------------------------------
static void copyAnimalCreaseVertices(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  MUintArray vertIds;
  MDoubleArray creaseData;
...
//----------------------------------------------------------------------------------------------------------------------
static void copyCreaseVertices(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  MUintArray vertIds;
  MDoubleArray creaseData;
  MStatus status = fnMesh.getCreaseVertices(vertIds, creaseData);
  if(status && creaseData.length() && vertIds.length())
  {
    ...
    doubleToFloat(subdCornerSharpnesses.data(), &creaseData[0], creaseData.length());
...
//----------------------------------------------------------------------------------------------------------------------
static void copyAnimalCreaseEdges(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  MUintArray edgeIds;
  MDoubleArray creaseData;
...
//----------------------------------------------------------------------------------------------------------------------
static void copyCreaseEdges(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  MUintArray edgeIds;
  MDoubleArray creaseData;
  MStatus status = fnMesh.getCreaseEdges(edgeIds, creaseData);
  if (status && edgeIds.length() && creaseData.length())
  {
    UsdPrim prim = mesh.GetPrim();
    {
      ...
      usdCreaseValues.resize(creaseData.length());
      doubleToFloat(usdCreaseValues.data(), (double*)&creaseData[0], creaseData.length());
...
// @todo: needs refactoring to handle face/vert/faceVarying correctly, allow separate RGB/A to be written etc.
static void copyAnimalFaceColours(UsdGeomMesh& mesh, const MFnMesh& fnMesh)
{
  MStringArray colourSetNames;
  MStatus status = fnMesh.getColorSetNames(colourSetNames);
  if(status == MS::kSuccess && colourSetNames.length())
...
//----------------------------------------------------------------------------------------------------------------------
void MeshTranslator::copyVertexData(const MFnMesh& fnMesh, const UsdAttribute& pointsAttr, UsdTimeCode time)
{
  MStatus status;
...
UsdPrim MeshTranslator::exportObject(UsdStageRefPtr stage, MDagPath path, const SdfPath& usdPath, const ExporterParams& params)
{
  ...
  if(status)
  {
    ...
    {
      copyAnimalFaceColours(mesh, fnMesh);
      copyAnimalCreaseVertices(mesh, fnMesh);
      copyAnimalCreaseEdges(mesh, fnMesh);
      copyGlimpseTesselationAttributes(mesh, fnMesh);
    }
    else
    {
      copyColourSetData(mesh, fnMesh);
      copyCreaseVertices(mesh, fnMesh);
      copyCreaseEdges(mesh, fnMesh);
    }
...
MObject MeshTranslator::createNode(const UsdPrim& from, MObject parent, const char* nodeType, const ImporterParams& params)
{
  ...
  applyHoleFaces(mesh, fnMesh);
  if(!applyVertexCreases(mesh, fnMesh))
  {
    applyAnimalVertexCreases(from, fnMesh);
  }
  if(!applyEdgeCreases(mesh, fnMesh))
  {
    applyAnimalEdgeCreases(from, fnMesh);
  }
