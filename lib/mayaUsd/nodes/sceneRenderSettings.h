...
/*! \brief Singleton DG node that holds an in-memory USD Stage for scene-level render settings.
 *
 ...
 *  As a pure DG node it has no DAG transform and does not appear in DAG
 *  traversals or the Outliner.  Its USD stage data is serialized into the Maya
 *  scene file so no external USD files are needed.
 *
