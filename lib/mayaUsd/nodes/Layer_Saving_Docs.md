...
the layer manager node exists only for teh brief moment when the Maya scene is
...
Layer Manager is set in its "layerManager" attribute. The "layerManagerReady"
attribute of the layer manager is connected to the corresponding attribute on
the Proxy Shape.
...
been loaded yet. That is where the `layerManagerReady` connection comes into
play. When the Layer Manager is loaded, eventually its connection to the Proxy
Shape will be restored. This will eventually trigger another `compute` for the
Proxy Shape, and at that point it will successfully find the Layer Manager.

In order to avoid loading data from teh Layer Manager multiple times, which
