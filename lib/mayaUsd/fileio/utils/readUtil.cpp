    ...
    if (newValue.IsHolding<TfToken>()) {
        ...
        if (Converter::hasAttrType(attrPlug, MFnData::kMatrix)) {
            ...
            for (size_t i = 0; i < 4; ++i) {
                for (size_t j = 0; j < 4; ++j) { mayaMat[i][j] = mat[i][j]; }
            }
            MFnMatrixData data;
        ...
        if (Converter::hasAttrType(attrPlug, MFnData::kStringArray)) {
            ...
            for (const TfToken& tok : result) { mayaArr.append(tok.GetText()); }
        ...
        if (Converter::hasAttrType(attrPlug, MFnData::kStringArray)) {
            ...
            MStringArray mayaArr;
            for (const std::string& str : result) { mayaArr.append(str.c_str()); }
            MFnStringArrayData data;
            MObject            dataObj = data.create();
            data.set(mayaArr);
            modifier.newPlugValue(attrPlug, dataObj);
        ...
        if (Converter::hasAttrType(attrPlug, MFnData::kStringArray)) {
            ...
            MStringArray mayaArr;
            for (const TfToken& tok : arr) { mayaArr.append(tok.GetText()); }
            MFnStringArrayData data;
            MObject            dataObj = data.create();
            data.set(mayaArr);
            modifier.newPlugValue(attrPlug, dataObj);
        ...
        if (Converter::hasAttrType(attrPlug, MFnData::kStringArray)) {
            ...
            MStringArray  mayaArr;
            for (const std::string& str : arr) { mayaArr.append(str.c_str()); }
            MFnStringArrayData data;
            MObject            dataObj = data.create();
            data.set(mayaArr);
            modifier.newPlugValue(attrPlug, dataObj);
        ...
        if (Converter::hasAttrType(attrPlug, MFnData::kVectorArray)) {
            ...
            for (const GfVec3d& v : arr) { mayaArr.append(MVector(v[0], v[1], v[2])); }
            ...
            VtVec3dArray arr = newValue.Get<VtVec3dArray>();
            MPointArray  mayaArr;
            for (const GfVec3d& v : arr) { mayaArr.append(MPoint(v[0], v[1], v[2])); }
