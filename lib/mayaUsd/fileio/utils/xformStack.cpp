...
TF_DEFINE_PUBLIC_TOKENS(UsdMayaXformStackTokens, PXRUSDMAYA_XFORM_STACK_TOKENS);

namespace {
const UsdGeomXformOp::Type RotateOpTypes[] = {
    UsdGeomXformOp::TypeRotateXYZ, UsdGeomXformOp::TypeRotateXZY, UsdGeomXformOp::TypeRotateYXZ,
...
{
    return (
        opType == UsdGeomXformOp::TypeRotateXYZ || opType == UsdGeomXformOp::TypeRotateXZY
        || opType == UsdGeomXformOp::TypeRotateYXZ || opType == UsdGeomXformOp::TypeRotateYZX
        || opType == UsdGeomXformOp::TypeRotateZXY || opType == UsdGeomXformOp::TypeRotateZYX);
}
...
_buildInversionMap(const std::vector<UsdMayaXformStack::IndexPair>& inversionTwins)
{
...
_makeInversionIndexPair(size_t opIndex, const UsdMayaXformStack::IndexMap& inversionMap)
{
...
UsdMayaXformStack::TokenIndexPairMap _buildAttrNamesToIdxs(
    ...
    const UsdMayaXformStack::IndexMap&    inversionMap)
{
    ...
    for (size_t i = 0; i < ops.size(); ++i) {
        ...
        // Insert, and check if it already existed
        TF_VERIFY(
            result.insert({ op.GetName(), indexPair }).second,
...
public:
    ...
    {
        return (
            m_name == other.m_name && m_opType == other.m_opType
...
UsdMayaXformOpClassification const& UsdMayaXformOpClassification::NullInstance()
{
    static UsdMayaXformOpClassification theNull;
    return theNull;
}

bool UsdMayaXformOpClassification::IsNull() const { return !_sharedData; }

TfToken const& UsdMayaXformOpClassification::GetName() const { return _sharedData->m_name; }

UsdGeomXformOp::Type UsdMayaXformOpClassification::GetOpType() const
{
    return _sharedData->m_opType;
}

bool UsdMayaXformOpClassification::IsInvertedTwin() const { return _sharedData->m_isInvertedTwin; }

std::vector<TfToken> UsdMayaXformOpClassification::CompatibleAttrNames() const
{
    // Note that we make tokens immortal because UsdMayaXformOpClassification
    // is currently not publicly creatable, and is only used to make
    ...
    if (_isThreeAxisRotate(GetOpType())) {
        ...
        if (GetName() == UsdMayaXformStackTokens->rotate) {
            ...
            for (UsdGeomXformOp::Type rotateType : RotateOpTypes) {
                ...
                result.emplace_back(TfToken(
                    UsdGeomXformOp::GetOpName(rotateType, GetName()).GetString(),
                    TfToken::Immortal));
            }
        }
    } else {
        ...
        result.emplace_back(TfToken(
            UsdGeomXformOp::GetOpName(GetOpType(), GetName()).GetString(), TfToken::Immortal));
        if (GetName() == UsdGeomXformOp::GetOpTypeToken(GetOpType())) {
            // Add, ie, xformOp::translate
            result.emplace_back(
                TfToken(UsdGeomXformOp::GetOpName(GetOpType()).GetString(), TfToken::Immortal));
        }
    }
    return result;
}

bool UsdMayaXformOpClassification::IsCompatibleType(UsdGeomXformOp::Type otherType) const
{
    if (GetOpType() == otherType)
        return true;
    if (_isThreeAxisRotate(GetOpType())) {
...
// a compile-time #define, with better type safety! Instead, it seems it still creates a
// full-fledged linker symbol...
...
public:
    ...
    _Data(
        ...
        , m_attrNamesToIdxs(_buildAttrNamesToIdxs(m_ops, m_inversionMap))
        , m_opNamesToIdxs(_buildOpNamesToIdxs(m_ops, m_inversionMap))
        , m_nameMatters(nameMatters)
    {
        ...
        for (auto& pair : m_inversionTwins) {
            ...
            TF_VERIFY(
                ...
                pair.first,
                first.GetName().GetText(),
                pair.second,
                second.GetName().GetText());
            TF_VERIFY(
                ...
                first.GetName().GetText());
            TF_VERIFY(
                first.IsInvertedTwin() != second.IsInvertedTwin(),
    ...
    inline const UsdMayaXformStack::OpClass& GetOpClassFromIndex(const size_t i) const
    {
        return i == UsdMayaXformStack::NO_INDEX ? UsdMayaXformStack::OpClass::NullInstance()
                                                : m_ops[i];
    }

    inline UsdMayaXformStack::OpClassPair
    MakeOpClassPairFromIndexPair(const UsdMayaXformStack::IndexPair& indexPair) const
    {
        return std::make_pair(
            GetOpClassFromIndex(indexPair.first), GetOpClassFromIndex(indexPair.second));
    }
...
UsdMayaXformStack::OpClassList const& UsdMayaXformStack::GetOps() const
{
    return _sharedData->m_ops;
}

std::vector<UsdMayaXformStack::IndexPair> const& UsdMayaXformStack::GetInversionTwins() const
{
    return _sharedData->m_inversionTwins;
}

bool UsdMayaXformStack::GetNameMatters() const { return _sharedData->m_nameMatters; }

UsdMayaXformOpClassification const& UsdMayaXformStack::operator[](const size_t index) const
{
    return _sharedData->m_ops[index];
}

size_t UsdMayaXformStack::GetSize() const { return _sharedData->m_ops.size(); }

size_t UsdMayaXformStack::FindOpIndex(const TfToken& opName, bool isInvertedTwin) const
{
    const UsdMayaXformStack::IndexPair& foundIdxPair = FindOpIndexPair(opName);
    ...
    if (firstOp.IsInvertedTwin()) {
        if (isInvertedTwin)
            return foundIdxPair.first;
        else
            return foundIdxPair.second;
    } else {
        if (isInvertedTwin)
            return foundIdxPair.second;
        else
...
const UsdMayaXformStack::IndexPair& UsdMayaXformStack::FindOpIndexPair(const TfToken& opName) const
{
    static UsdMayaXformStack::IndexPair _NO_MATCH = std::make_pair(NO_INDEX, NO_INDEX);
...
const UsdMayaXformStack::OpClassPair UsdMayaXformStack::FindOpPair(const TfToken& opName) const
{
    return _sharedData->MakeOpClassPairFromIndexPair(FindOpIndexPair(opName));
}

UsdMayaXformStack::OpClassList
UsdMayaXformStack::MatchingSubstack(const std::vector<UsdGeomXformOp>& xformops) const
{
...
UsdMayaXformStack::OpClassList UsdMayaXformStack::FirstMatchingSubstack(
    ...
    const std::vector<UsdGeomXformOp>&           xformops)
{
    ...
    for (auto& stackPtr : stacks) {
        std::vector<UsdMayaXformStack::OpClass> stackOps = stackPtr->MatchingSubstack(xformops);
        if (!stackOps.empty()) {
            return stackOps;
        }
    }
    ...
    static UsdMayaXformStack commonStack(
        ...
        { UsdMayaXformOpClassification(
          ...
          UsdMayaXformOpClassification(
              UsdMayaXformStackTokens->rotateZ, UsdGeomXformOp::TypeRotateZ),
          UsdMayaXformOpClassification(UsdMayaXformStackTokens->scale, UsdGeomXformOp::TypeScale),
          UsdMayaXformOpClassification(UsdMayaXformStackTokens->scaleX, UsdGeomXformOp::TypeScaleX),
          UsdMayaXformOpClassification(UsdMayaXformStackTokens->scaleY, UsdGeomXformOp::TypeScaleY),
          UsdMayaXformOpClassification(UsdMayaXformStackTokens->scaleZ, UsdGeomXformOp::TypeScaleZ),
          UsdMayaXformOpClassification(
              UsdMayaXformStackTokens->pivot,
              UsdGeomXformOp::TypeTranslate,
              true /* isInvertedTwin */) },
...
const UsdMayaXformStack& UsdMayaXformStack::MatrixStack()
{
    static UsdMayaXformStack matrixStack(
        // ops
        { UsdMayaXformOpClassification(
            UsdMayaXformStackTokens->transform, UsdGeomXformOp::TypeTransform) },
