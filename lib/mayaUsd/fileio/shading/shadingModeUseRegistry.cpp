...
private:
    ...
    UsdMayaShaderWriterSharedPtr _GetShaderWriterForNode(
        ...
        _NodeHandleToShaderWriterMap&          shaderWriterMap)
    {
        ...
        UsdMayaPrimWriterSharedPtr primWriter
            = context.GetWriteJobContext().CreatePrimWriter(depNodeFn, shaderUsdPath);
        ...
        const UsdMayaShadingModeExportContext& context)
    {
        ...
        for (; !iterDepGraph.isDone(); iterDepGraph.next()) {
            ...
            UsdMayaShaderWriterSharedPtr srcShaderWriter = _GetShaderWriterForNode(
                srcPlug.node(), material.GetPath(), context, shaderWriterMap);
            if (!srcShaderWriter) {
                continue;
            }
            ...
            for (unsigned int i = 0u; i < dstPlugs.length(); ++i) {
                ...
                UsdMayaShaderWriterSharedPtr dstShaderWriter = _GetShaderWriterForNode(
                    dstPlug.node(), material.GetPath(), context, shaderWriterMap);
                if (!dstShaderWriter) {
                    continue;
                }
                ...
                const TfToken srcPlugName = TfToken(context.GetStandardAttrName(srcPlug, false));
                UsdAttribute  srcAttribute
                    = srcShaderWriter->GetShadingAttributeForMayaAttrName(srcPlugName);

                const TfToken dstPlugName = TfToken(context.GetStandardAttrName(dstPlug, false));
                UsdAttribute  dstAttribute
                    = dstShaderWriter->GetShadingAttributeForMayaAttrName(dstPlugName);
        ...
        SdfPathSet* const                      boundPrimPaths) override
    {
        ...
        UsdShadeShader surfaceShaderSchema
            = _ExportShadingDepGraph(material, context.GetSurfaceShaderPlug(), context);
        ...
        UsdShadeShader volumeShaderSchema
            = _ExportShadingDepGraph(material, context.GetVolumeShaderPlug(), context);
        UsdMayaShadingUtil::CreateShaderOutputAndConnectMaterial(
            volumeShaderSchema,
        ...
        UsdShadeShader displacementShaderSchema
            = _ExportShadingDepGraph(material, context.GetDisplacementShaderPlug(), context);
        UsdMayaShadingUtil::CreateShaderOutputAndConnectMaterial(
...
TF_REGISTRY_FUNCTION_WITH_TAG(UsdMayaShadingModeExportContext, useRegistry)
{
    UsdMayaShadingModeRegistry::GetInstance().RegisterExporter(
        "useRegistry", []() -> UsdMayaShadingModeExporterPtr {
