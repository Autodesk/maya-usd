...
UsdMayaShadingModeExportContext::GetAssignments() const
{
    ...
    for (unsigned int i = 0; i < dsmPlug.numConnectedElements(); i++) {
        ...
        if (SdfPathFindLongestPrefix(
                ...
                _bindableRoots, usdPath)
            == _bindableRoots.end()) {
#else
                _bindableRoots.begin(), _bindableRoots.end(), usdPath)
            == _bindableRoots.end()) {
#endif
            continue;
        }
        ...
        TF_FOR_ALL(iter, assignmentsToBind)
        {
            ...
            if (faceIndices.empty()) {
                ...
                UsdGeomSubset faceSubset
                    = UsdShadeMaterialBindingAPI(boundPrim).CreateMaterialBindSubset(
                        /* subsetName */ TfToken(materialName),
                        faceIndices,
                        /* elementType */ UsdGeomTokens->face);

                if (!GetExportArgs().exportCollectionBasedBindings) {
                    UsdShadeMaterialBindingAPI(faceSubset.GetPrim()).Bind(material);
                }
                ...
                UsdShadeMaterialBindingAPI(boundPrim).SetMaterialBindSubsetsFamilyType(
...
std::string UsdMayaShadingModeExportContext::GetStandardAttrName(
    const MPlug& plug,
