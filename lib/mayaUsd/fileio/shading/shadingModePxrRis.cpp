...
private:
        ...
        SdfPathSet*                            processedPaths)
    {
        ...
        for (const auto& attrPlug : inputAttrPlugs) {
            ...
            const TfToken attrName = TfToken(context.GetStandardAttrName(attrPlug, false));
            if (attrName.IsEmpty()) {
                continue;
            }
...
TF_REGISTRY_FUNCTION_WITH_TAG(UsdMayaShadingModeExportContext, pxrRis)
{
    UsdMayaShadingModeRegistry::GetInstance().RegisterExporter(
        "pxrRis", []() -> UsdMayaShadingModeExporterPtr {
...
static TfToken _GetMayaTypeNameForShaderId(const TfToken& shaderId)
{
