    ...
    UsdMayaPrimReaderContext*    context)
{
    ...
    for (size_t i = 0; i < usdTimes.size(); ++i) { mayaTimes[i] = usdTimes[i]; }
    ...
    for (size_t jointIdx = 0; jointIdx < jointNodes.size(); ++jointIdx) {
        ...
        for (size_t i = 0; i < samples.size(); ++i) { xforms[i] = samples[i][jointIdx]; }
    ...
    unsigned int            numPoints)
{
    ...
    for (unsigned int i = 0; i < numJoints; ++i) { influenceIndices[i] = i; }
