...
MAYAUSD_NS_DEF
{
        ...
        , m_pointsNumTimeSamples(0u)
    {
        ...
        if (fvc.ValueMightBeTimeVarying()) {
            // at some point, it would be great, instead of failing, to create a usd/hydra proxy
            // node for the mesh, perhaps?  For now, better to give a more specific error
            TF_RUNTIME_ERROR(
        ...
        if (normals.size() == static_cast<size_t>(meshFn.numFaceVertices())) {
            for (size_t i = 0u; i < polygonCounts.length(); ++i) {
                for (int j = 0; j < polygonCounts[i]; ++j) { normalsFaceIds.append(i); }
            }
    ...
    MStatus TranslatorMeshRead::setPointBasedDeformerForMayaNode(
        const MObject& mayaObj, const MObject& stageNode, const UsdPrim& prim)
    {
        ...
        const std::string pointBasedDeformerNodeName = TfStringPrintf(
            ...
            TfStringReplace(
                prim.GetPath().GetString(), SdfPathTokens->childDelimiter.GetString(), "_")
                .c_str());
    ...
    MString TranslatorMeshRead::pointBasedDeformerName() const
    {
        ...
        const MDagPath&          dagPath)
    {
        ...
        if (!TF_VERIFY(
                m_usdMesh, "Could not define UsdGeomMesh at path '%s'\n", usdPath.GetText())) {
            return;
        }
    }
