...
MAYAUSD_NS_DEF
{
    ...
    std::vector<std::string> UsdAttributes::attributeNames() const
    {
        ...
        for (const auto& attr : primAttrs) { names.push_back(attr.GetName()); }
        return names;
    }
    ...
    UsdAttributes::getUfeTypeForAttribute(const PXR_NS::UsdAttribute& usdAttr) const
    {
        ...
        static const std::unordered_map<size_t, Ufe::Attribute::Type>
            sUsdTypeToUfe {
        ...
        if (usdAttr.IsValid()) {
            ...
            if (iter != sUsdTypeToUfe.end()) {
                ...
                if (iter->second == Ufe::Attribute::kEnumString) {
                    ...
                    auto attrDefn
                        = fPrim.GetPrimDefinition().GetSchemaAttributeSpec(usdAttr.GetName());
#else
