...
UsdStageMap::StageSet UsdStageMap::allStages()
{
    ...
    for (auto it = fPathToObject.begin(); it != fPathToObject.end(); /* no ++it here, we manually move it in the loop */) {
        const auto& pair = *it;
        ...
        // After this point pair and path cannot be safely used, the may have been erased.
