...
MAYAUSD_NS_DEF
{
    ...
    template <> void TypedUsdAttribute<std::string>::set(const std::string& value)
    {
        ...
        if (typeName.GetHash() == SdfValueTypeNames->String.GetHash()) {
            std::string dummy;
            UFE_ASSERT_MSG(
                fUsdAttr.Get<std::string>(&dummy, getCurrentTime(sceneItem())),
                kErrorMsgInvalidType);
    ...
    template <> void TypedUsdAttribute<Ufe::Color3f>::set(const Ufe::Color3f& value)
    {
        GfVec3f vec;
        UFE_ASSERT_MSG(
            fUsdAttr.Get<GfVec3f>(&vec, getCurrentTime(sceneItem())), kErrorMsgInvalidType);
    ...
    template <typename T> void TypedUsdAttribute<T>::set(const T& value)
    {
        T dummy;
        UFE_ASSERT_MSG(
            fUsdAttr.Get<T>(&dummy, getCurrentTime(Ufe::Attribute::sceneItem())),
            kErrorMsgInvalidType);
