...
MAYAUSD_NS_DEF
{
    ...
    /*
        ...
        SdfPrimSpec::SetName() will rename any prim in the layer, but it does not allow you to
       reparent the prim, nor will it update any relationship or connection targets in the layer
       that targeted the prim or any of its decendants (they will all break unless you fix them up
       yourself.Renaming and reparenting prims destructively in composed scenes is pretty tricky
       stuff that cannot really practically be done with 100% guarantees.
    */
    ...
    bool UsdUndoRenameCommand::renameRedo()
    {
        ...
        for (auto child : prim.GetParent().GetChildren()) { childrenNames.insert(child.GetName()); }
        ...
        // SdfLayer is a "simple" container, and all it knows about defaultPrim is that it is a
        // piece of token-valued layer metadata. It is only the higher-level Usd and Pcp modules
        // that know that it is identifying a prim on the stage. One must use the SdfLayer API for
        // setting the defaultPrim when you rename the prim it identifies.
    ...
    bool UsdUndoRenameCommand::renameUndo()
    {
        ...
        // shouldn't have to again create a sibling sceneItem here since we already have a valid
        // _ufeSrcItem however, I get random crashes if I don't which needs furthur investigation.
        // HS, 6-May-2020.
