...
bool inAttributeChangedNotificationGuard() { return attributeChangedNotificationGuardCount > 0; }
...
MAYAUSD_NS_DEF
{
        ...
        UsdStageWeakPtr const&           sender)
    {
        ...
        for (const auto& changedPath : notice.GetResyncedPaths()) {
            ...
            Ufe::Path ufePath
                = stagePath(sender) + Ufe::PathSegment(usdPrimPathStr, g_USDRtid, '/');
    ...
    void StagesSubject::onStageInvalidate(const MayaUsdProxyStageInvalidateNotice& notice)
    {
        ...
        Ufe::SceneItem::Ptr sceneItem
            = Ufe::Hierarchy::createItem(notice.GetProxyShape().ufePath());
