...
Ufe::Transform3d::Ptr createTransform3d(const Ufe::SceneItem::Ptr& item)
{
    ...
    return UsdUfe::UsdTransform3dSetObjectMatrix::create(
        editTransform3d, ml.GetInverse(), GfMatrix4d(1));
}
...
Ufe::Matrix4d UsdTransform3dFallbackMayaXformStack::segmentExclusiveMatrix() const
{
    ...
    auto local = UsdUfe::computeLocalExclusiveTransform(ops, findFirstFallbackOp(ops), time);
    return UsdUfe::toUfe(local * parent);
}
