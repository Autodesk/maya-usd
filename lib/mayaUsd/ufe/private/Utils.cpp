...
MAYAUSD_NS_DEF
{
    ...
    void applyCommandRestriction(const UsdPrim& prim, const std::string& commandName)
    {
        ...
        if (!MayaUsdUtils::hasSpecs(prim)) {
            std::string err = TfStringPrintf(
                "Cannot %s [%s] because it doesn't have any specs to contribute to the composed "
                "prim.",
                commandName.c_str(),
        ...
        if (!MayaUsdUtils::doesEditTargetLayerContribute(prim)) {
            ...
            std::string err = TfStringPrintf(
                "Cannot %s [%s] defined on another layer. "
            ...
            if (layers.size() > 1) {
                ...
                std::string err = TfStringPrintf(
                    "Cannot %s [%s] with definitions or opinions on other layers. "
                    "Opinions exist in %s",
                    commandName.c_str(),
                    prim.GetName().GetString().c_str(),
            ...
            try {
                ...
                if (!primXform.SetTranslate(GfVec3d(x, y, z)))
                    throw std::runtime_error(
                        "Unable to SetTranslate after conversion to CommonAPI.");
    ...
    void
    rotatePivotTranslateOp(const UsdPrim& prim, const Ufe::Path& path, double x, double y, double z)
    {
        auto primXform = UsdGeomXformCommonAPI(prim);
