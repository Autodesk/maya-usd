...
const char targetLayerPrimPathAttrName[] = "usdStageTargetLayerPrimPath";
...
MStatus copyTargetLayerToAttribute(const PXR_NS::UsdStage& stage, MayaUsdProxyShapeBase& proxyShape)
{
    ...
    getDynamicAttribute(depNode, targetLayerPrimPathAttrName, previousTargetLayerPrimPathText);
    ...
    if (status == MS::kSuccess && targetLayerPrimPath.length()) {
        status = setDynamicAttribute(depNode, targetLayerPrimPathAttrName, targetLayerPrimPath);
    }
    ...
    PXR_NS::UsdStage&            stage)
{
    ...
    MString targetLayerText;
    if (getDynamicAttribute(depNode, targetLayerAttrName, targetLayerText) != MS::kSuccess) {
        return {};
    }
    ...
    if (hasDynamicAttribute(depNode, targetLayerPrimPathAttrName)) {
        getDynamicAttribute(depNode, targetLayerPrimPathAttrName, targetLayerPrimPathText);
    }
