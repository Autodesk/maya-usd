...
// Debuggging helpers to log message to help diagnose problems.
...
// Function called for every SdfSpec starting from a prim to be copied.
// Copies each prim encountered and optionally adds the targets of
// relationships to list of other paths to also be copied.
//
    ...
    MayaUsd::CopyLayerPrimsResult&        result)
{
    ...
    for (const auto& srcAndDest : result.copiedPaths) {
        ...
        if (pathToCopy.HasPrefix(alreadyDone)) {
            ...
            // Note: it may have been copied indirectly, in that case we still want
            //       to add it to the list of copied paths.
            if (result.copiedPaths.count(pathToCopy) == 0) {
                ...
                //
                // Note: break at the first repalcement, to avoid double-renaming,
                //       in the case there was a chain of renaming. For example:
                //
                //       Let's say we copied a1 and a2, respectively containing a
                //       b1 and b2. Suppose the destination already contained a1.
                //
                //       Then a1 will become a2 and a2 will become a3 in the destination.
                //       When detecting that a1/b1 was laready copied, we want to correctly
                //       rename it to a2/b1, but then avoid renaming it again to a3/b1.
                //
                //       That is why we interrupt renaming at the first renaming.

                for (const auto& oldAndNew : result.renamedPaths) {
                    const SdfPath& oldPrefix = oldAndNew.first;
                    if (dstPath.HasPrefix(oldPrefix)) {
                        const SdfPath& newPrefix = oldAndNew.second;
                        dstPath = dstPath.ReplacePrefix(oldPrefix, newPrefix);
                        break;
                    }
                }
...
auto makeRenameTraverser(
    const UsdStageRefPtr&                dstStage,
    const MayaUsd::CopyLayerPrimsResult& result)
{
    auto renameRelFn = [&dstStage, &result](const SdfPath& layerSpecPath) -> bool {
        return renameTraverser(dstStage, layerSpecPath, result);
    };
    return renameRelFn;
}
    ...
    const CopyLayerPrimsOptions& options)
{
    ...
    // to the list so to copy the related prims.
    ...
    auto renameRelFn = makeRenameTraverser(dstStage, result);

    addProgressSteps(options, result.copiedPaths.size());
    for (const auto& srcAndDest : result.copiedPaths) {
        const SdfPath& dstPath = srcAndDest.second;
        if (result.renamedPaths.count(dstPath)) {
            traverseLayer(dstLayer, result.renamedPaths[dstPath], renameRelFn);
        } else {
            traverseLayer(dstLayer, dstPath, renameRelFn);
        }
        advanceProgress(options);
    }
