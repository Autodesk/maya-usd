...
struct MAYAUSD_CORE_PUBLIC ConverterArgs final
{
    ...
    //! For types supporting gamma correction, following argument controls if it should be applied
    bool        _doGammaCorrection{true};
};

/*! \brief Performant converter, providing several interfaces to translated between Maya and Usd data types.
 
    Instances of converter class are initialized once for all supported types. To use the converter, use static
    find methods to validate if the required converter is available. Pointer to converter class is safe to cache
    in acceleration structures to reduce runtime cost when the same conversion happens often (e.g. every frame).
 
    The converter will avoid using expensive or intermediate conversion like for example type-erased Get method
    on Usd attribute. This means VtValue is not used as an intermediate type unless it's the source or destination type.
 
    The converter takes destination containers as a reference to allow fetching the data to the same container.
 
    Type checking should be performed when retrieving data converter, no extra penalty at runtime should be necessary.
...
public:
    ...
    //! \brief  Construct new converter. This should never be called directly other than during generation of all supported converters.
    ...
    //! \brief  Look for converter which allows translation for given pair of Maya's plug and Usd attribute.
    //! \return Valid pointer if conversion is supported.
    ...
    //! \brief  Read current value from given attribute and set it on destination plug. Use arguments to control converter behavior.
    void convert(const UsdAttribute& src, MPlug& dst, const ConverterArgs& args) const
    { _attrToPlug(src,dst,args); }
    //! \brief  Read current value from given attribute and set it on destination plug using provided DG modifier. Use arguments to control converter behavior.
    ...
    //! \brief  Read current value from given plug and set it on destination VtValue. Use arguments to control converter behavior.
    void convert(const MPlug& src, VtValue& dst, const ConverterArgs& args) const
    { _plugToVtValue(src, dst, args); }
    //! \brief  Read current value from given VtValue and set it on destination plug. Use arguments to control converter behavior.
    void convert(const VtValue& src, MPlug& dst, const ConverterArgs& args) const
    { _vtValueToPlug(src, dst, args); }
    //! \brief  Read current value from given VtValue and set it on destination plug using provided DG modifier. Use arguments to control converter behavior.
    ...
    //! \brief  Read current value from given data handle and set it on destination attribute. Use arguments to control converter behavior, like the time for setting value on attribute.
    void convert(const MDataHandle& src, UsdAttribute& dst, const ConverterArgs& args) const
    { _handleToAttr(src,dst,args); }
    //! \brief  Read current value from given attribute and set it on destination data handle. Use arguments to control converter behavior, like the time for reading value from attribute.
    void convert(const UsdAttribute& src, MDataHandle& dst, const ConverterArgs& args) const
    { _attrToHandle(src,dst,args); }
    
    //! \brief  Read current value from given data handle and set it on destination VtValue. Use arguments to control converter behavior.
    void convert(const MDataHandle& src, VtValue& dst, const ConverterArgs& args) const
    { _handleToVtValue(src, dst, args); }
    //! \brief  Read current value from given VtValue and set it on destination data handle. Use arguments to control converter behavior.
...
private:
    ...
    //! Pointer to a function responsible for converting usd attribute value to a given plug using provided DG modifier.
    ...
    //! Pointer to a function responsible for converting VtValue to a given plug using provided DG modifier.
    VtValueToMDGModifierFn _vtValueToModifier{nullptr};
    
    // MDataBlock <--> UsdAttribute
    //! Pointer to a function responsible for converting data handle value to given usd attribute.
    MDataHandleToUsdAttrFn _handleToAttr{nullptr};
    //! Pointer to a function responsible for converting usd attribute value to a given data handle.
...
//! \brief  Declaration of base typed converter struct. Each supported type will specialize this struct and provide
//!         two conversion methods. All specializations will be available in converter header file to allow reuse
//!         when both types are already known.
template <class MAYA_Type, class USD_Type, class Enable=void> struct TypedConverter {};

//! \brief  Specialization of TypedConverter for case where MAYA_Type and USD_Type are exactly the same type.
