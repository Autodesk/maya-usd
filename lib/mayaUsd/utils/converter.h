...
namespace MAYAUSD_NS_DEF
{
    ...
    */
    class MAYAUSD_CORE_PUBLIC Converter final {
        ...
        using VtValueToMDGModifierFn = std::add_pointer<
            void(const VtValue&, MPlug&, MDGModifier&, const ConverterArgs&)>::type;
    ...
    public:
        ...
        //! provided DG modifier. Use arguments to control converter behavior.
        void
        convert(const VtValue& src, MPlug& plug, MDGModifier& dst, const ConverterArgs& args) const
        {
    ...
    template <> struct TypedConverter<MIntArray, VtArray<int>> {
        static void convert(const VtArray<int>& src, MIntArray& dst)
        {
            ...
            for (size_t i = 0; i < srcSize; i++) { dst[i] = src[i]; }
        }
