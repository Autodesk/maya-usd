...
namespace MAYAUSD_NS_DEF
{
    ...
    struct MDataHandleUtils<
        MAYA_Type,
        typename std::enable_if<MakeMayaFnData<MAYA_Type>::value>::type> {
...
// we should be able to back-port the change to an update.
    ...
    struct Converter::GenerateConverters {
        ...
        static ConvertStorage generate()
        {
            ...
            createConverter<MPointArray, VtArray<GfVec3f>>(
                converters, SdfValueTypeNames->Point3fArray);
    ...
    struct Converter::GenerateArrayPlugConverters {
        ...
        template <
            ...
            NeedsGammaCorrection ColorCorrection = NeedsGammaCorrection::kNo>
        static void
        createArrayConverter(ConvertStorage& converters, const SdfValueTypeName& typeName)
        {
            converters.emplace(
                typeName,
                Converter(
    ...
    SdfValueTypeName Converter::getUsdTypeName(
        const MPlug& attrPlug, const bool translateMayaDoubleToUsdSinglePrecision)
    {
