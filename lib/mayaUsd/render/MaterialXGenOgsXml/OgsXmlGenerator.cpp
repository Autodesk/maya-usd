...
static const std::string VARYING_INPUT_PARAM = "varyingInputParam",
...
void xmlAddImplementation(
    pugi::xml_node& parent,
    const string&   language,
    const string&   languageVersion,
...
void xmlSetProperty(
    ...
    const string&       parameterFlags = EMPTY_STRING,
    const pugi::char_t* refNode = nullptr)
{
    ...
    const pugi::char_t*  refNode = nullptr)
{
    for (size_t i = 0; i < block.size(); ++i) {
        ...
        if (refNode
            && (shaderPort->getVariable() == DIFFUSEI || shaderPort->getVariable() == SPECULARI)) {
        ...
        if (shaderPort->getType() == Type::FILENAME) {
            ...
            if (!textureName.empty()) {
                ...
                xmlSetProperty(
                    texture, shaderPort->getName(), textureName, parameterFlags, refNode);
            ...
            if (type != getOgsTypeMap().end()) {
                ...
                if (shaderPort->getType() == Type::MATRIX33) {
                    const string var = shaderPort->getVariable()
        ...
        if (p->getValue()) {
            ...
            if (type != getOgsTypeMap().end()) {
                ...
                if (p->getType() == Type::MATRIX33) {
                    ...
                    string var
                        = p->getVariable() + GlslFragmentGenerator::MATRIX3_TO_MATRIX4_POSTFIX;
    ...
    const std::string& hlslSource)
{
    ...
    xmlAddProperties(
        xmlProperties,
        glslPixelStage.getUniformBlock(HW::PRIVATE_UNIFORMS),
        OgsParameterFlags::IS_REQUIREMENT_ONLY);
    ...
    xmlAddImplementation(
        xmlImpementations,
        "GLSL",
        "3.0",
        glslPixelStage.getFunctionName(),
        glslShader.getSourceCode(Stage::PIXEL));
    xmlAddImplementation(
        xmlImpementations, "HLSL", "11.0", glslPixelStage.getFunctionName(), hlslSource);
    ...
    const Shader&      glslShader)
{
    ...
    xmlConnection.append_attribute(FROM)
        = DOT_COMBINE(LIGHT_BUILDER, OgsXmlGenerator::OUTPUT_NAME.c_str()).c_str();
    ...
    xmlConnection.append_attribute(TO)
        = DOT_COMBINE(baseShaderName.c_str(), LIGHTLOOPRESULT).c_str();
    ...
    xmlAddProperties(
        xmlProperties,
        glslPixelStage.getUniformBlock(HW::PUBLIC_UNIFORMS),
        EMPTY_STRING,
        baseShaderName.c_str());
    xmlAddProperties(
        xmlProperties,
