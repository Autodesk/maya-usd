    ...
    case HdInterpolationVertex:
        if (requiresUnsharedVertices) {
            ...
            if (numVertices == faceVertexIndices.size()) {
                ...
                TF_CODING_ERROR(
                    "Invalid Hydra prim '%s': primvar %s "
...
unsigned int _GetNumOfEdgeIndices(const HdMeshTopology& topology)
{
    ...
    for (std::size_t i = 0; i < faceVertexCounts.size(); i++) { numIndex += faceVertexCounts[i]; }
    ...
    const TfToken&   reprToken)
{
    ...
    if (HdChangeTracker::IsTopologyDirty(*dirtyBits, id)) {
        ...
        if (requiresUnsharedVertices) {
            ...
            _meshSharedData._unsharedTopology.reset(new HdMeshTopology(
                topology.GetScheme(),
    ...
    if (HdChangeTracker::IsPrimvarDirty(*dirtyBits, id, HdTokens->points)) {
        ...
        const bool requiresUnsharedVertices = _meshSharedData._unsharedTopology != nullptr;

        const size_t numVertices = requiresUnsharedVertices ? topology.GetFaceVertexIndices().size()
    ...
    bool                  requireFlatNormals)
{
    ...
    if (desc.geomStyle == HdMeshGeomStyleHull) {
        ...
        if (((itemDirtyBits & HdChangeTracker::DirtyPrimvar) != 0)
            ...
            if (colorInterp == HdInterpolationConstant && alphaInterp == HdInterpolationConstant) {
        ...
        if (0 == instanceCount) {
            ...
            if (const auto state = drawScene.GetActiveSelectionState(id)) {
                for (const auto& indexArray : state->instanceIndices) {
                    for (const auto index : indexArray) { colorIndices[index] = 1; }
                }
            }
            ...
            if (const auto state = drawScene.GetLeadSelectionState(id)) {
                for (const auto& indexArray : state->instanceIndices) {
                    for (const auto index : indexArray) { colorIndices[index] = 2; }
                }
            }
