...
{
    std::lock_guard<std::mutex> lock(_allInstancesMutex);
    for (auto* instance : _allInstances) { instance->_renderGlobalsHaveChanged = true; }
}
...
void MtohRenderOverride::_UpdateRenderDelegateOptions()
{
    ...
    const auto* settings = TfMapLookupPtr(_globals.rendererSettings, _rendererDesc.rendererName);
    if (settings == nullptr) {
        return;
    }
