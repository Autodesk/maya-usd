...
#endif // defined(MAYAUSD_USE_SHARED_LAYER_EDITOR)
...
class BaseCmdWrapper : public Ufe::UndoableCommand
{
public:
    BaseCmdWrapper(std::shared_ptr<BaseCmd> cmd, SdfLayerHandle layer)
        : _cmd(std::move(cmd))
        , _layer(std::move(layer))
    {
    }

    void redo() override { _cmd->doIt(_layer); }
    void undo() override { _cmd->undoIt(_layer); }

    // Access to _cmdResult for AddAnonSubLayer.
    const std::string& cmdResult() const { return _cmd->_cmdResult; }

    // Returns true if the underlying command is an AddAnonSubLayer.
    bool isAddAnonSubLayer() const
    {
        return dynamic_cast<AddAnonSubLayer*>(_cmd.get()) != nullptr;
    }

private:
    std::shared_ptr<BaseCmd> _cmd;
...
MStatus LayerEditorCommand::parseArgs(const MArgList& argList)
{
    ...
    if (!isQuery()) {
...
#else // !defined(MAYAUSD_USE_SHARED_LAYER_EDITOR)
        ...
        auto makeWrapper = [&layer](std::shared_ptr<Impl::BaseCmd> cmd)
            -> std::shared_ptr<Impl::BaseCmdWrapper> {
            return std::make_shared<Impl::BaseCmdWrapper>(std::move(cmd), layer);
        };
        ...
        if (argParser.isFlagSet(kInsertSubPathFlag)) {
            ...
            for (unsigned i = 0; i < count; i++) {
                ...
                _subCommands.push_back(makeWrapper(std::move(cmd)));
            }
        }
        ...
        if (argParser.isFlagSet(kDiscardEditsFlag)) {
            _subCommands.push_back(makeWrapper(std::make_shared<Impl::DiscardEdit>()));
        }

        if (argParser.isFlagSet(kClearLayerFlag)) {
            _subCommands.push_back(makeWrapper(std::make_shared<Impl::ClearLayer>()));
        }

        if (argParser.isFlagSet(kFlattenLayerFlag)) {
            _subCommands.push_back(makeWrapper(std::make_shared<Impl::FlattenLayer>()));
        }
        ...
        if (argParser.isFlagSet(kStitchLayersFlag)) {
            ...
            auto cmd = std::make_shared<Impl::StitchLayers>(
                layerIdentifiers, proxyShapeName.asChar());
            _subCommands.push_back(makeWrapper(std::move(cmd)));
        }
...
#else
        if (auto* wrapper = dynamic_cast<Impl::BaseCmdWrapper*>(cmd.get())) {
            if (wrapper->isAddAnonSubLayer())
                appendToResult(wrapper->cmdResult().c_str());
        }
#endif
    }
    return MS::kSuccess;
