...
static void addArcLayers(const UsdPrimCompositionQueryArc& arc, std::set<SdfLayerHandle>& layers)
{
    PcpNodeRef                 pcpNode = arc.GetTargetNode();
    const PcpLayerStackRefPtr& layerStack = pcpNode.GetLayerStack();
    for (const SdfLayerRefPtr& layer : layerStack->GetLayers()) {
        layers.insert(layer);
    }
}

std::vector<LayerAndPath> getAuthoredLayerAndPaths(const UsdPrim& prim, bool includePayloads)
{
    // The logic in thuis function is tailored for its uses, in particular
    // in the duplicate command (UsdUndoDuplicateCommand). It is a poor-man,
    // rough heuristic for a multi-layers workflow, which might be better
    // fleshed out in the future once we start to tackle multi-layers operations.
    //
    // For now, it makes the duplicate command work better in the presence of
    // references and payload, and in particular the difference between the
    // defining opinion containing a payload arc vs the defining opinion being
    // *in* a payload arc or reference. No claim that we are doing the right thing
    // in all possible combinations of opinions, payloads and refs, but this
    // handle the simple cases.

    // If we don't want to include data from within the payloads,
    ...
    // The defining layer must always be kept.
    SdfLayerHandle defLayer = MayaUsdUtils::getDefiningLayerAndPath(prim).layer;
    ...
    for (SdfPrimSpecHandle primSpec : prim.GetPrimStack()) {
        ...
        if (layer != defLayer)
            if (payloadLayers.count(layer) > 0)
                continue;

        layerAndPaths.emplace_back(layer, primSpec->GetPath());
    }
