...
#include <mayaHydraLib/delegates/proxyDelegate.h>
...
MayaHydraProxyAdapter::MayaHydraProxyAdapter(MayaHydraDelegateCtx* delegate, const MDagPath& dag)
    : MayaHydraShapeAdapter(delegate->GetPrimPath(dag, false), delegate, dag)
{
    ...
    TfWeakPtr<MayaHydraProxyAdapter> me(this);
    TfNotice::Register(me, &MayaHydraProxyAdapter::_OnStageSet);

    MayaHydraProxyDelegate::AddAdapter(this);
}

MayaHydraProxyAdapter::~MayaHydraProxyAdapter() { MayaHydraProxyDelegate::RemoveAdapter(this); }

void MayaHydraProxyAdapter::Populate()
{
    if (_isPopulated || !_proxy) {
        return;
    }

    TF_DEBUG(MAYAHYDRALIB_AL_POPULATE)
        .Msg("MayaHydraProxyDelegate::Populating %s\n", _proxy->name().asChar());
...
bool MayaHydraProxyAdapter::IsSupported() const { return _proxy != nullptr; }

void MayaHydraProxyAdapter::MarkDirty(HdDirtyBits dirtyBits)
{
    if (dirtyBits != 0) {
...
VtValue MayaHydraProxyAdapter::Get(const TfToken& key)
{
    TF_DEBUG(MAYAHYDRALIB_ADAPTER_GET)
        .Msg(
            "Called MayaHydraProxyAdapter::Get(%s) - %s\n",
            key.GetText(),
            GetDagPath().partialPathName().asChar());
    return {};
}

bool MayaHydraProxyAdapter::HasType(const TfToken& typeId) const { return false; }

void MayaHydraProxyAdapter::PopulateSelectedPaths(
...
void MayaHydraProxyAdapter::CreateUsdImagingDelegate()
{
    ...
    _usdDelegate.reset();
    _usdDelegate.reset(new MayaHydraProxyUsdImagingDelegate(
...
void MayaHydraProxyAdapter::_OnStageSet(const MayaUsdProxyStageSetNotice& notice)
{
    if (&notice.GetProxyShape() == _proxy) {
        // Real work done by delegate->createUsdImagingDelegate
        TF_DEBUG(MAYAHYDRALIB_AL_CALLBACKS)
            .Msg(
                "MayaHydraProxyAdapter - called StageLoadedCallback "
                "(ProxyShape: "
                "%s)\n",
                GetDagPath().partialPathName().asChar());
...
{
    TfType::Define<MayaHydraProxyAdapter, TfType::Bases<MayaHydraDagAdapter>>();
}

TF_REGISTRY_FUNCTION_WITH_TAG(MayaHydraAdapterRegistry, MayaUsd_ProxyShape)
{
    MayaHydraAdapterRegistry::RegisterShapeAdapter(
        ...
        [](MayaHydraDelegateCtx* delegate, const MDagPath& dag) -> MayaHydraShapeAdapterPtr {
            return MayaHydraShapeAdapterPtr(new MayaHydraProxyAdapter(delegate, dag));
        });
}
