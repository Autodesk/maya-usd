...
class MayaUsdPrimReaderMyMesh final : public UsdMayaPrimReader
{
public:
    MayaUsdPrimReaderMyMesh(const UsdMayaPrimReaderArgs& args)
        : UsdMayaPrimReader(args)
    {
    }

    ~MayaUsdPrimReaderMyMesh() override { }
...
TF_REGISTRY_FUNCTION_WITH_TAG(UsdMayaPrimReaderRegistry, UsdGeomMesh)
{
    UsdMayaPrimReaderRegistry::Register<UsdGeomMesh>(
        UsdMayaPrimReader::CanImport,
        [](const UsdMayaPrimReaderArgs& args) {
            return std::make_shared<MayaUsdPrimReaderMyMesh>(args);
        });
}
