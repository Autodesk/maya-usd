...
#include <hdMaya/adapters/renderItemDiscoveryPlugin.h>
...
{
    static PlugPluginPtr plugin = PlugRegistry::GetInstance().GetPluginWithName("hdMaya");
    const std::string path = PlugFindPluginResource(plugin, 
            TfStringCatPaths("resources", resourceName));
...
const NdrStringVec& 
HdMayaRenderItemShadersDiscoveryPlugin::GetSearchURIs() const
{
...
HdMayaRenderItemShadersDiscoveryPlugin::DiscoverNodes(const Context &context)
{
...
NDR_REGISTER_DISCOVERY_PLUGIN(HdMayaRenderItemShadersDiscoveryPlugin);
