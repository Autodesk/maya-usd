...
class HdMayaMesh2Adapter : public HdMayaShapeAdapter
{
public:
    HdMayaMesh2Adapter(HdMayaDelegateCtx* delegate, const MDagPath& dag)
        : HdMayaShapeAdapter(delegate->GetPrimPath(dag, false), delegate, dag)
    {
    }

    ~HdMayaMesh2Adapter() = default;
    ...
    VtValue Get(const TfToken& key) override
    {
        TF_DEBUG(HDMAYA_ADAPTER_GET)
            .Msg(
                "Called HdMayaMesh2Adapter::Get(%s) - %s\n",
                key.GetText(),
...
private:
    ...
    {
        // auto* adapter = reinterpret_cast<HdMayaMesh2Adapter*>(clientData);
        ...
        //         "HdMayaMesh2Adapter::NodeDirtiedCallback.\n",
        //         plug.name().asChar(),
        //         plug.partialName().asChar());
    }
        ...
        void*                          clientData)
    {
        ...
        //         .Msg(
        //             "%s (%s) plug dirtying was not handled by "
        //             "HdMayaMesh2Adapter::attributeChangedCallback.\n",
        //             plug.name().asChar(),
        //             plug.name().asChar());
        // }
    }
...
{
    TfType::Define<HdMayaMesh2Adapter, TfType::Bases<HdMayaShapeAdapter>>();
}
...
TF_REGISTRY_FUNCTION_WITH_TAG(HdMayaAdapterRegistry, mesh)
{
    HdMayaAdapterRegistry::RegisterShapeAdapter(
        ...
        [](HdMayaDelegateCtx* delegate, const MDagPath& dag) -> HdMayaShapeAdapterPtr {
            return HdMayaShapeAdapterPtr(new HdMayaMesh2Adapter(delegate, dag));
        });
}
