...
void _nodeAdded(MObject& obj, void* clientData)
{
    ...
    auto* delegate = reinterpret_cast<HdMayaScene2Delegate*>(clientData);
...
{
    TfType::Define<HdMayaScene2Delegate, TfType::Bases<HdMayaDelegate>>();
}

TF_REGISTRY_FUNCTION_WITH_TAG(HdMayaDelegateRegistry, HdMayaScene2Delegate)
{
    HdMayaDelegateRegistry::RegisterDelegate(
        _tokens->HdMayaScene2Delegate,
        [](const HdMayaDelegate::InitData& initData) -> HdMayaDelegatePtr {
            return std::static_pointer_cast<HdMayaDelegate>(
                std::make_shared<HdMayaScene2Delegate>(initData));
        });
}

HdMayaScene2Delegate::HdMayaScene2Delegate(const InitData& initData)
    : HdMayaDelegateCtx(initData)
    , _fallbackMaterial(initData.delegateID.AppendChild(_tokens->FallbackMaterial))
{
}

HdMayaScene2Delegate::~HdMayaScene2Delegate()
{
...
void HdMayaScene2Delegate::Populate()
{
...
void HdMayaScene2Delegate::PreFrame(const MHWRender::MDrawContext& context)
{
...
void HdMayaScene2Delegate::RemoveAdapter(const SdfPath& id)
{
    if (!_RemoveAdapter<HdMayaAdapter>(
            ...
            _lightAdapters,
            _materialAdapters)) {
        TF_WARN("HdMayaScene2Delegate::RemoveAdapter(%s) -- Adapter does not exists", id.GetText());
    }
}

void HdMayaScene2Delegate::RecreateAdapterOnIdle(const SdfPath& id, const MObject& obj)
{
    // TODO: Thread safety?
...
void HdMayaScene2Delegate::MaterialTagChanged(const SdfPath& id)
{
...
void HdMayaScene2Delegate::RebuildAdapterOnIdle(const SdfPath& id, uint32_t flags)
{
    // We expect this to be a small number of objects, so using a simple linear
    // search and a vector is generally a good choice.
...
void HdMayaScene2Delegate::RecreateAdapter(const SdfPath& id, const MObject& obj)
{
    if (_RemoveAdapter<HdMayaAdapter>(
            id,
            [](HdMayaAdapter* a) {
    ...
    if (_RemoveAdapter<HdMayaMaterialAdapter>(
        ...
        TF_WARN(
            "HdMayaScene2Delegate::RecreateAdapterOnIdle(%s) -- Adapter does "
            "not exists",
            id.GetText());
    }
}

HdMayaShapeAdapterPtr HdMayaScene2Delegate::GetShapeAdapter(const SdfPath& id)
{
    auto iter = _shapeAdapters.find(id);
    return iter == _shapeAdapters.end() ? nullptr : iter->second;
}

HdMayaLightAdapterPtr HdMayaScene2Delegate::GetLightAdapter(const SdfPath& id)
{
    auto iter = _lightAdapters.find(id);
    return iter == _lightAdapters.end() ? nullptr : iter->second;
}

HdMayaMaterialAdapterPtr HdMayaScene2Delegate::GetMaterialAdapter(const SdfPath& id)
{
    auto iter = _materialAdapters.find(id);
    return iter == _materialAdapters.end() ? nullptr : iter->second;
}

void HdMayaScene2Delegate::InsertDag(const MDagPath& dag)
{
    TF_DEBUG(HDMAYA_DELEGATE_INSERTDAG)
        .Msg(
            "HdMayaScene2Delegate::InsertDag::"
            "GetLightsEnabled()=%i\n",
            GetLightsEnabled());
...
void HdMayaScene2Delegate::NodeAdded(const MObject& obj) { _addedNodes.push_back(obj); }

void HdMayaScene2Delegate::UpdateLightVisibility(const MDagPath& dag)
{
    const auto id = GetPrimPath(dag, true);
    _FindAdapter<HdMayaLightAdapter>(
        id,
...
void HdMayaScene2Delegate::AddNewInstance(const MDagPath& dag)
{
    MDagPathArray dags;
...
void HdMayaScene2Delegate::SetParams(const HdMayaParams& params)
{
    ...
    const HdSelectionSharedPtr& selection)
{
    ...
    MapSelectionDescendents(
        ...
        [this, &selectedSdfPaths, &selectedMasters, &selection](const MDagPath& dagPath) {
            ...
            TF_DEBUG(HDMAYA_DELEGATE_SELECTION)
                .Msg(
                    "HdMayaScene2Delegate::PopulateSelectedPaths - calling "
...
void HdMayaScene2Delegate::PopulateSelectionList(
...
HdMeshTopology HdMayaScene2Delegate::GetMeshTopology(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_MESH_TOPOLOGY)
        .Msg("HdMayaScene2Delegate::GetMeshTopology(%s)\n", id.GetText());
...
HdBasisCurvesTopology HdMayaScene2Delegate::GetBasisCurvesTopology(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_CURVE_TOPOLOGY)
        .Msg("HdMayaScene2Delegate::GetBasisCurvesTopology(%s)\n", id.GetText());
...
PxOsdSubdivTags HdMayaScene2Delegate::GetSubdivTags(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_SUBDIV_TAGS)
        .Msg("HdMayaScene2Delegate::GetSubdivTags(%s)\n", id.GetText());
...
GfRange3d HdMayaScene2Delegate::GetExtent(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_EXTENT).Msg("HdMayaScene2Delegate::GetExtent(%s)\n", id.GetText());
    return _GetValue<HdMayaShapeAdapter, GfRange3d>(
        id, [](HdMayaShapeAdapter* a) -> GfRange3d { return a->GetExtent(); }, _shapeAdapters);
}

GfMatrix4d HdMayaScene2Delegate::GetTransform(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_TRANSFORM)
        .Msg("HdMayaScene2Delegate::GetTransform(%s)\n", id.GetText());
...
size_t HdMayaScene2Delegate::SampleTransform(
    const SdfPath& id,
    ...
    GfMatrix4d*    samples)
{
    TF_DEBUG(HDMAYA_DELEGATE_SAMPLE_TRANSFORM)
        .Msg(
            "HdMayaScene2Delegate::SampleTransform(%s, %u)\n",
            id.GetText(),
...
bool HdMayaScene2Delegate::IsEnabled(const TfToken& option) const
{
    TF_DEBUG(HDMAYA_DELEGATE_IS_ENABLED)
        .Msg("HdMayaScene2Delegate::IsEnabled(%s)\n", option.GetText());
    ...
    TF_WARN("HdMayaScene2Delegate::IsEnabled(%s) -- Unsupported option.\n", option.GetText());
    return false;
}

VtValue HdMayaScene2Delegate::Get(const SdfPath& id, const TfToken& key)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET)
        .Msg("HdMayaScene2Delegate::Get(%s, %s)\n", id.GetText(), key.GetText());
...
size_t HdMayaScene2Delegate::SamplePrimvar(
    const SdfPath& id,
    const TfToken& key,
    size_t         maxSampleCount,
    float*         times,
    VtValue*       samples)
{
    TF_DEBUG(HDMAYA_DELEGATE_SAMPLE_PRIMVAR)
        .Msg(
            "HdMayaScene2Delegate::SamplePrimvar(%s, %s, %u)\n",
            id.GetText(),
            key.GetText(),
            static_cast<unsigned int>(maxSampleCount));
    if (maxSampleCount < 1) {
...
TfToken HdMayaScene2Delegate::GetRenderTag(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_RENDER_TAG)
        .Msg("HdMayaScene2Delegate::GetRenderTag(%s)\n", id.GetText());
...
HdMayaScene2Delegate::GetPrimvarDescriptors(const SdfPath& id, HdInterpolation interpolation)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_PRIMVAR_DESCRIPTORS)
        .Msg("HdMayaScene2Delegate::GetPrimvarDescriptors(%s, %i)\n", id.GetText(), interpolation);
    if (id.IsPropertyPath()) {
...
VtValue HdMayaScene2Delegate::GetLightParamValue(const SdfPath& id, const TfToken& paramName)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_LIGHT_PARAM_VALUE)
        .Msg(
            "HdMayaScene2Delegate::GetLightParamValue(%s, %s)\n", id.GetText(), paramName.GetText());
...
VtIntArray
HdMayaScene2Delegate::GetInstanceIndices(const SdfPath& instancerId, const SdfPath& prototypeId)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_INSTANCE_INDICES)
        .Msg(
            "HdMayaScene2Delegate::GetInstanceIndices(%s, %s)\n",
            instancerId.GetText(),
            prototypeId.GetText());
...
SdfPath HdMayaScene2Delegate::GetInstancerId(const SdfPath& primId)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_INSTANCER_ID)
        .Msg("HdMayaScene2Delegate::GetInstancerId(%s)\n", primId.GetText());
...
GfMatrix4d HdMayaScene2Delegate::GetInstancerTransform(SdfPath const& instancerId)
{
    return GfMatrix4d(1.0);
}

#if defined(HD_API_VERSION) && HD_API_VERSION >= 34
SdfPath HdMayaScene2Delegate::GetScenePrimPath(
...
SdfPath HdMayaScene2Delegate::GetScenePrimPath(const SdfPath& rprimPath, int instanceIndex)
{
    return rprimPath;
}
#else
SdfPath HdMayaScene2Delegate::GetPathForInstanceIndex(
...
bool HdMayaScene2Delegate::GetVisible(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_VISIBLE)
        .Msg("HdMayaScene2Delegate::GetVisible(%s)\n", id.GetText());
...
bool HdMayaScene2Delegate::GetDoubleSided(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_DOUBLE_SIDED)
        .Msg("HdMayaScene2Delegate::GetDoubleSided(%s)\n", id.GetText());
    return _GetValue<HdMayaShapeAdapter, bool>(
        id, [](HdMayaShapeAdapter* a) -> bool { return a->GetDoubleSided(); }, _shapeAdapters);
}

HdCullStyle HdMayaScene2Delegate::GetCullStyle(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_CULL_STYLE)
        .Msg("HdMayaScene2Delegate::GetCullStyle(%s)\n", id.GetText());
    return HdCullStyleDontCare;
}

HdDisplayStyle HdMayaScene2Delegate::GetDisplayStyle(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_DISPLAY_STYLE)
        .Msg("HdMayaScene2Delegate::GetDisplayStyle(%s)\n", id.GetText());
...
SdfPath HdMayaScene2Delegate::GetMaterialId(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_MATERIAL_ID)
        .Msg("HdMayaScene2Delegate::GetMaterialId(%s)\n", id.GetText());
    if (!_enableMaterials)
        return {};
...
VtValue HdMayaScene2Delegate::GetMaterialResource(const SdfPath& id)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_MATERIAL_RESOURCE)
        .Msg("HdMayaScene2Delegate::GetMaterialResource(%s)\n", id.GetText());
...
HdTextureResource::ID HdMayaScene2Delegate::GetTextureResourceID(const SdfPath& textureId)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_TEXTURE_RESOURCE_ID)
        .Msg("HdMayaScene2Delegate::GetTextureResourceID(%s)\n", textureId.GetText());
...
HdTextureResourceSharedPtr HdMayaScene2Delegate::GetTextureResource(const SdfPath& textureId)
{
    TF_DEBUG(HDMAYA_DELEGATE_GET_TEXTURE_RESOURCE)
        .Msg("HdMayaScene2Delegate::GetTextureResource(%s)\n", textureId.GetText());
...
bool HdMayaScene2Delegate::_CreateMaterial(const SdfPath& id, const MObject& obj)
{
    TF_DEBUG(HDMAYA_ADAPTER_MATERIALS)
        .Msg("HdMayaScene2Delegate::_CreateMaterial(%s)\n", id.GetText());
