...
MAYAUSD_NS_DEF
{
        ...
        QWidget * parent, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        if (type == kVariants) {
            ...
            if (nullptr != treeItem) {
                // Look in the import data (from the tree model) to see if it has a prim variant
                // selection matching the prim from this item. If found we send that variant
                // selection to the editor so that it will set that value rather (in the combobox)
                // rather than the one from prim.
                ...
                if ((treeModel != nullptr) && (treeModel->importData() != nullptr)) {
                    const ImportData::PrimVariantSelections& primVarSel
                        = treeModel->importData()->primVariantSelections();
                    ImportData::PrimVariantSelections::const_iterator iter
                        = primVarSel.find(treeItem->prim().GetPath());
                    if (iter != std::end(primVarSel)) {
                        varSelMap = iter->second;
                    }
                }

                VariantsEditorWidget* editor
                    = new VariantsEditorWidget(parent, this, treeItem->prim(), varSelMap);
                if (!varSelMap.empty()) {
                    setModelData(
                        editor, treeItem->model(), treeItem->model()->indexFromItem(treeItem));
                }
                return editor;
            }
        }
    ...
    {
        // Draw gridlines - doesn't work correctly as it sometimes draws lines in the middle of
        // cells!
        // 	painter->save();
        // 	painter->setPen(QColor(60, 60, 60)/*option.palette.color(QPalette::Base)*/);
        // 	painter->drawRect(QRect(0, option.rect.y(), option.rect.right(),
        ...
        if (proxyModel != nullptr) {
            ...
            if (treeModel != nullptr) {
                TreeItem* item = static_cast<TreeItem*>(
                    treeModel->itemFromIndex(proxyModel->mapToSource(index)));
                return item;
            }
        }
    ...
    VariantsEditorWidget::VariantsEditorWidget(
        QWidget * parent,
        ...
        : QWidget(parent)
    {
        ...
        for (auto it = usdVarSetNames.crbegin(); it != usdVarSetNames.crend(); it++) {
            ...
            for (const auto& vn : usdVarNames) { qtVarNames.push_back(vn.c_str()); }
    ...
    QLayout* VariantsEditorWidget::createVariantSet(
        const ItemDelegate* itemDelegate, const QString& varName, QStringList& varNames)
    {
    ...
    {
        QStringList names;
        for (auto lbl : fVariantLabels) { names.append(lbl->text()); }
        return names;
    }
    ...
    {
        QStringList sels;
        for (auto cb : fVariantCombos) { sels.append(cb->currentText()); }
        return sels;
    }
