    ...
    const QModelIndex&          index) const
{
    ...
    if (type == kVariants) {
        ...
        if (nullptr != treeItem) {
            ...
            // selection matching the prim from this item. If found we send that variant selection
            // to the editor so that it will set that value rather (in the combobox) rather than the
            // one from prim.
...
TreeItem* ItemDelegate::getTreeItemForIndex(const QModelIndex& index) const
{
    ...
    if (proxyModel != nullptr) {
        ...
        if (treeModel != nullptr) {
            TreeItem* item
                = static_cast<TreeItem*>(treeModel->itemFromIndex(proxyModel->mapToSource(index)));
            return item;
        }
    }
