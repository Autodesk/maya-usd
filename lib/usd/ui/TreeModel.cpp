...
MAYAUSD_NS_DEF
{
    ...
    void TreeModel::fillStagePopulationMask(
        UsdStagePopulationMask & popMask, const QModelIndex& parent)
    {
        for (int r = 0; r < rowCount(parent); ++r) {
            // Note: only the load column (0) has children, so we use it when looking for children.
            QModelIndex childIndex = this->index(r, kTreeColumn_Load, parent);
    ...
    void TreeModel::fillPrimVariantSelections(
        ImportData::PrimVariantSelections & primVariantSelections, const QModelIndex& parent)
    {
        for (int r = 0; r < rowCount(parent); ++r) {
            ...
            if (item->variantSelectionModified()) {
                ...
                if (varSel.isValid() && varSel.canConvert<QStringList>()) {
                    ...
                    QStringList varNames
                        = variantIndex.data(ItemDelegate::kVariantNameRole).toStringList();
                    assert(varSelections.count() == varNames.count());
                    if (varSelections.count() == varNames.count()) {
                        SdfVariantSelectionMap varSels;
                        for (int i = 0; i < varNames.count(); ++i) {
                            varSels[varNames.at(i).toStdString()]
                                = varSelections.at(i).toStdString();
                        }
        ...
        for (int r = 0; r < rowCount(parent); ++r) {
            ...
            if (type == ItemDelegate::kVariants) {
                QSortFilterProxyModel* proxyModel
                    = qobject_cast<QSortFilterProxyModel*>(tv->model());
