...
MAYAUSD_NS_DEF
{
    ...
    //
    // While additional traits like std::is_copy_constructible or std::is_move_constructible could
    // also be used, the fact that the Factory cannot be (traditionally) instantiated prevents other
    ...
    QList<QStandardItem*> TreeModelFactory::createPrimRow(const UsdPrim& prim)
    {
        ...
        // (despite it being cached and optimized for frequent access). Avoiding frequent
        // conversions from USD Strings to Qt Strings helps in keeping memory allocations low.
        ...
        size_t&                      insertionsRemaining)
    {
        ...
        if (primShouldBeIncluded) {
            ...
            // Only continue processing additional USD Prims if all expected results have not
            // already been found:
