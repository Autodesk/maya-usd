...
//
#ifndef USDLAYEREDITOR_LAYERLOCKING_H
#define USDLAYEREDITOR_LAYERLOCKING_H
...
/*! Map the original layer name when the scene was saved to the current layer name.
    Layer renaming happens when anonymous layers are saved within the DCC scene file.
*/
 ...
 * @param locked The layer identifiers of layers to be locked.
 * @param nameMap Layer name map. When Anon layers are saved to a DCC scene, and reloaded,
 * their names change, this map allows to handle the remapping that needs to be done to lock the
 * layers.
