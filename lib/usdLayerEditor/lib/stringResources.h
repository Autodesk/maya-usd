...
LAYEREDITOR_UI_PUBLIC Resource create(const char* key, const char* value);
...
const auto kReloadTitle                  { create("kReloadTitle", "Reload \"^1s\"") };
const auto kReloadMsg                    { create("kReloadMsg", "Reloading \"^1s\" will discard all current edits and revert it to its state on disk. Are you sure you want to proceed?") };
const auto kReloadButtonText             { create("kReloadButtonText", "Reload") };
