...
#ifndef USDLAYEREDITOR_COMPONENTSAVEWIDGET_H
#define USDLAYEREDITOR_COMPONENTSAVEWIDGET_H
 ...
 */
class LAYEREDITOR_UI_PUBLIC ComponentSaveWidget : public QWidget
{
    Q_OBJECT
...
public:
    ComponentSaveWidget(
        ...
        SessionState*      in_sessionState = nullptr,
        const std::string& dccObjectPath = std::string());
    ...
    // Get the DCC object path (proxy shape path on the Maya side)
    const std::string& dccObjectPath() const { return _dccObjectPath; }
...
#endif // USDLAYEREDITOR_COMPONENTSAVEWIDGET_H
