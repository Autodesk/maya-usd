...
- **Muted-layer registry is unified on the shared store.** Since the `mayaUsdLayerEditor` command now dispatches muting through `UsdLayerEditor::MuteLayerCmd`, held muted layers are stored only in `lib/usdLayerEditor/lib/layerMuting.cpp`. That store was made keyed by muted-layer identifier (`unordered_map<identifier, LayerRefSet>`) and gained `getMutedLayers(const std::string&)`, mirroring the former Maya store. `lib/mayaUsd/utils/layerMuting.cpp` is now a thin forwarder to it (plus the Maya-only proxy-shape attribute bridge and the scene-reset listener). This fixes `testLayerManagerSerialization` (`testSaveMutedRootSubLayer{ToMaya,ToUsd}`): serialization reads the same registry the command writes, so dirty content under a muted root sub-layer survives save/reopen/unmute.
