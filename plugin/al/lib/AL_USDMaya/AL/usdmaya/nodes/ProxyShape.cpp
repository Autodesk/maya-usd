    ...
    const fileio::translators::TranslatorParameters& param)
{
    ...
    if (TfDebug::IsEnabled(ALUSDMAYA_TRANSLATORS)) {
        ...
        for (auto it : filter.updatablePrimSet()) { std::cout << it.GetPath().GetText() << '\n'; }
        std::cout << "removed prims" << std::endl;
        for (auto it : filter.removedPrimSet()) { std::cout << it.GetText() << '\n'; }
    }
...
void ProxyShape::constructGLImagingEngine()
{
    ...
    if (MGlobal::mayaState() == MGlobal::kInteractive) {
        if (m_stage) {
            ...
            for (auto& it : translatedGeo) { excludedGeometryPaths.push_back(it.second); }
