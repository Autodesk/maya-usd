...
MStatus ProxyShapeImport::doIt(const MArgList& args)
{
    ...
    for (size_t i = 0; i < dagPaths.length(); i++) { stringNames.append(getPath(dagPaths[i])); }
    clearResult();
    ...
    try {
        ...
        if (m_recursive) {
            ...
            for (auto importPath : m_importPaths) {
                ...
                for (auto it : newPrimSet) { newImportPaths.push_back(it.GetPath()); }
            }
            ...
            for (auto teardownPath : m_teardownPaths) {
                ...
                for (auto it : newPrimSet) { newTeardownPaths.push_back(it.GetPath()); }
            }
