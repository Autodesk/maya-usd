...
struct ImportCallback {
    enum ScriptType : uint32_t { kMel, kPython };
    ...
    AL_BEGIN_PROFILE_SECTION(CreatePrims);
    {
        ...
        for (; it != end; ++it) {
            ...
            // if(!context->hasEntry(prim.GetPath(), prim.GetTypeName()))
            {
                ...
                for (auto dataPlugin : dataPlugins) { dataPlugin->import(prim, created); }
            }
        }
    }
    AL_END_PROFILE_SECTION();
                ...
                if (translator) {
                    if (translator->update(prim).statusCode() == MStatus::kNotImplemented) {
                        ...
                        if (context->getMObjects(prim, returned) && !returned.empty()) {
                            ...
                            for (auto dataPlugin : dataPlugins) { dataPlugin->update(prim); }
                        }
                    }
                }
            }
    ...
    for (; it != end; ++it) {
        ...
        if (torBase) {
            ...
            if (context->getMObjects(prim, returned) && !returned.empty()) {
                ...
                for (auto dataPlugin : dataPlugins) { dataPlugin->postImport(prim); }
            }
