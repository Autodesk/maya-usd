...
//----------------------------------------------------------------------------------------------------------------------
/// \brief  A Maya export plugin that writes out USD files from Maya (this is largely optimised for
/// the needs of the
...
static constexpr const char* const kDynamicAttributes
    = "Dynamic Attributes"; ///< export dynamic attributes option name
static constexpr const char* const kDuplicateInstances
...
static constexpr const char* const kSubSamples
    = "Sub Samples"; ///< specify the number of sub samples to export
static constexpr const char* const kFilterSample
    = "Filter Sample"; ///< export filter sample option name
static constexpr const char* const kExportAtWhichTime
    = "Export At Which Time"; ///< which time code should be used for default values?
static constexpr const char* const kExportInWorldSpace
    = "Export In World Space"; ///< should selected transforms be output in world space?
static constexpr const char* const kActivateAllTranslators
    = "Activate all Plugin Translators"; ///< if true, all translator plugins will be enabled by
                                         ///< default
static constexpr const char* const kActiveTranslatorList
    = "Active Translator List"; ///< A comma seperated list of translator plugins that should be
                                ///< activated for export
static constexpr const char* const kInactiveTranslatorList
    = "Inactive Translator List"; ///< A comma seperated list of translator plugins that should be
                                  ///< inactive for export
...
static MStatus specifyOptions(AL::maya::utils::FileTranslatorOptions& options)
{
    ...
    if (!options.addFrame("AL USD Exporter Options"))
        return MS::kFailure;
    if (!options.addBool(kDynamicAttributes, defaultValues.m_dynamicAttributes))
        return MS::kFailure;
    ...
    if (!options.addBool(kUseTimelineRange, defaultValues.m_useTimelineRange))
        return MS::kFailure;
    if (!options.addFloat(kFrameMin, defaultValues.m_minFrame))
        return MS::kFailure;
    if (!options.addFloat(kFrameMax, defaultValues.m_maxFrame))
        return MS::kFailure;
    if (!options.addInt(kSubSamples, defaultValues.m_subSamples))
        return MS::kFailure;
    if (!options.addBool(kFilterSample, defaultValues.m_filterSample))
        return MS::kFailure;
    if (!options.addEnum(kExportAtWhichTime, timelineLevel, defaultValues.m_exportAtWhichTime))
        return MS::kFailure;
    if (!options.addBool(kExportInWorldSpace, defaultValues.m_exportInWorldSpace))
        return MS::kFailure;
...
{
    if (!options.removeFrame("AL USD Exporter Options"))
        return MS::kFailure;
...
{
    // I need to possibly recreate this when dirty (i.e. when new optins have been
