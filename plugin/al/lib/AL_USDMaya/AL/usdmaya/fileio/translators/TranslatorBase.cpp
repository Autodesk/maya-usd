...
{
    for (auto& it : m_translatorsMap) { it.second->activate(); }
}

//----------------------------------------------------------------------------------------------------------------------
void TranslatorManufacture::deactivateAll()
{
    for (auto& it : m_translatorsMap) { it.second->deactivate(); }
}
...
void TranslatorManufacture::preparePythonTranslators(TranslatorContext::RefPtr context)
{
    ...
    for (auto it : TranslatorManufacture::m_pythonTranslators) { it->setContext(context); }
