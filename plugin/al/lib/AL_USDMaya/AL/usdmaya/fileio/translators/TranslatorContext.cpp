...
MString TranslatorContext::serialise() const
{
    ...
    for (auto& path : m_excludedGeometry) { oss << path.first.GetString() << ","; }
...
void TranslatorContext::deserialise(const MString& string)
{
    ...
    for (auto& it : vec) { m_excludedGeometry.emplace(it, it); }
}
    ...
    if (stage) {
        ...
        if (translator) {
            ...
            if (prim) {
                ...
                for (auto dataPlugin : dataPlugins) { dataPlugin->preTearDown(prim); }
