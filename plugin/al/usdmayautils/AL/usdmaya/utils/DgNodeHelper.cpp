    ...
    const size_t      count)
{
    ...
    for (size_t i = 0; i != count; ++i) { plug.elementByLogicalIndex(i).setBool(values[i]); }
    ...
    const size_t        count)
{
    ...
    for (size_t i = 0; i != count; ++i) { plug.elementByLogicalIndex(i).setChar(values[i]); }
    ...
    const size_t         count)
{
    ...
    for (size_t i = 0; i != count; ++i) { plug.elementByLogicalIndex(i).setShort(values[i]); }
    ...
    const size_t         count)
{
    ...
    for (size_t i = 0; i != count; ++i) { plug.elementByLogicalIndex(i).setValue(values[i]); }
    ...
    const size_t         count)
{
    ...
    for (size_t i = 0; i != count; ++i) { plug.elementByLogicalIndex(i).setInt64(values[i]); }
...
#endif
#else
            for (int k = 0; k < 16; ++k) { dptr[k] = fptr[k]; }
#endif
        }
    ...
    if (plug.isDestination()) {
        ...
        if (sourcePlug.node().hasFn(MFn::kAnimCurve)) {
            ...
            if (isAnimCurveTypeSupported(animCurveFn)) {
                ...
                for (int i = keyNumber - 1; i >= 0; --i) { animCurveFn.remove(i); }
                return MS::kSuccess;
            }
        }
...
DgNodeHelper::getBoolArray(const MObject& node, const MObject& attr, std::vector<bool>& values)
{
    ...
    values.resize(num);
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asBool(); }
    return MS::kSuccess;
}
...
DgNodeHelper::getBoolArray(MObject node, MObject attribute, bool* const values, const size_t count)
{
    ...
    for (uint32_t i = count16; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asBool(); }
#else
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asBool(); }
#endif
...
#else
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asInt64(); }
#endif
...
#else
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asInt(); }
#endif
    ...
    const size_t  count)
{
    ...
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asChar(); }
    ...
    const size_t   count)
{
    ...
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asShort(); }
...
#else
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asFloat(); }
#endif
    return MS::kSuccess;
}
...
#else
    for (uint32_t i = 0; i < num; ++i) { values[i] = plug.elementByLogicalIndex(i).asDouble(); }
#endif
    return MS::kSuccess;
}
    ...
    const size_t count)
{
    ...
    if (plug.isArray()) {
        ...
        for (uint32_t i = 0, n = fn.length(); i < n; ++i) {
            ...
            storeu4f(optr + 12, m3);
#else
            for (int k = 0; k < 16; ++k) { optr[k] = iptr[k]; }
#endif
        }
    }
    return MS::kSuccess;
...
#endif
#else
            for (uint32_t k = 0; k < 16; ++k) { optr[k] = iptr[k]; }
#endif
        }
    }
    ...
    const UsdTimeCode& timeCode)
{
    ...
    case UsdDataType::kFloat:
        if (!isArray) {
            ...
            m.resize(plug.numElements());
            getFloatArray(node, attribute, (float*)m.data(), m.size());
            for (auto it = m.begin(), e = m.end(); it != e; ++it) { *it *= scale; }
            usdAttr.Set(m, timeCode);
        }
        break;
    ...
    case UsdDataType::kDouble:
        if (!isArray) {
            ...
            getDoubleArray(node, attribute, (double*)m.data(), m.size());
            double temp = scale;
            for (auto it = m.begin(), e = m.end(); it != e; ++it) { *it *= temp; }
            usdAttr.Set(m, timeCode);
        }
        break;
