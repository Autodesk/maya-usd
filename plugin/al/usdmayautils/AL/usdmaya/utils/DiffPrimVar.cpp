...
uint32_t diffGeom(UsdGeomPointBased& geom, MFnMesh& mesh, UsdTimeCode timeCode, uint32_t exportMask)
{
  ...
  if(exportMask & kPoints)
  {
    ...
    if(!MayaUsd::Utils::compareArray(usdPoints, mayaPoints, usdPointsCount * 3, mayaPointsCount * 3))
    {
      result |= kPoints;
    }
  }
  ...
  if(exportMask & kNormals)
  {
    ...
    else
    {
      ...
      if(!MayaUsd::Utils::compareArray(usdNormals, mayaNormals, usdNormalsCount * 3, mayaNormalsCount * 3))
      {
        result |= kNormals;
      }
    }
...
uint32_t diffFaceVertices(UsdGeomMesh& geom, MFnMesh& mesh, UsdTimeCode timeCode, uint32_t exportMask)
{
  ...
  if(exportMask & (kFaceVertexCounts | kFaceVertexIndices))
  {
    ...
    if(numPolygons == faceVertexCounts.size() && numFaceVerts == faceVertexIndices.size())
    {
      ...
      if(numPolygons && !MayaUsd::Utils::compareArray(&vertexCount[0], pFaceVertexCounts, numPolygons, numPolygons))
      {
        result |= kFaceVertexCounts;
      }

      const int* const pFaceVertexIndices = faceVertexIndices.cdata();
      if(numFaceVerts && !MayaUsd::Utils::compareArray(&vertexList[0], pFaceVertexIndices, numFaceVerts, numFaceVerts))
      {
  ...
  if(exportMask & kHoleIndices)
  {
    ...
    if(numMayaHoleIndices && !MayaUsd::Utils::compareArray((int32_t*)&mayaHoleIndices[0], holeIndices.cdata(), numMayaHoleIndices, numHoleIndices))
    {
      result |= kHoleIndices;
    }
  }
  ...
  if(exportMask & (kCreaseWeights | kCreaseIndices))
  {
    ...
    if(exportMask & kCreaseIndices)
    {
      ...
      if(numMayaCreaseIndices && !MayaUsd::Utils::compareArray((const int32_t*)&mayaCreaseIndices[0], creasesIndices.cdata(), numMayaCreaseIndices, numCreaseIndices))
      {
        result |= kCreaseIndices;
      }
    }
    ...
    if(exportMask & kCreaseWeights)
    {
      ...
      if(numMayaCreaseWeights && !MayaUsd::Utils::compareArray(&mayaCreaseWeights[0], creasesWeights.cdata(), numMayaCreaseWeights, numCreaseWeights))
      {
        result |= kCreaseWeights;
      }
    }
  ...
  if(exportMask & (kCornerIndices | kCornerSharpness))
  {
    ...
    {
      if(numMayaVertexIds && !MayaUsd::Utils::compareArray((const int32_t*)&mayaVertexIdValues[0], vertexIdValues.cdata(), numVertexIds, numMayaVertexIds))
      {
        result |= kCornerIndices;
      }
    }
    ...
    {
      if(numMayaCreaseValues && !MayaUsd::Utils::compareArray(&mayaCreaseValues[0], creaseValues.cdata(), numCreaseValues, numMayaCreaseValues))
      {
        result |= kCornerSharpness;
      }
    }
      ...
      if (definition.m_primVar.Get(&vtValue, UsdTimeCode::Default()))
      {
        if (vtValue.IsHolding<VtArray<GfVec2f> >())
        {
          ...
          if(definition.m_interpolation == UsdGeomTokens->faceVarying)
          {
            ...
            if(!MayaUsd::Utils::compareUvArray(
    ...
    MIntArray& pointIndices)
{
  ...
  if(MayaUsd::Utils::vec2AreAllTheSame(&u[0], &v[0], u.length()))
  {
    return UsdGeomTokens->constant;
  }

  // if the UV indices match the vertex indices, we have per-vertex assignment
  if(MayaUsd::Utils::compareArray(&indices[0], &pointIndices[0], indices.length(), pointIndices.length()))
  {
...
{
  // if UV coords are all identical, we have a constant value
  if(MayaUsd::Utils::vec3AreAllTheSame(xyz, numElements))
  {
    return UsdGeomTokens->constant;
  }
...
{
  // if UV coords are all identical, we have a constant value
  if(MayaUsd::Utils::vec4AreAllTheSame(xyzw, numElements))
  {
    return UsdGeomTokens->constant;
  }
...
{
  // if prim vars are all identical, we have a constant value
  if(MayaUsd::Utils::vec4AreAllTheSame(rgba, numElements))
  {
    return UsdGeomTokens->constant;
  }
