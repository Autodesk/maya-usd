...
{
    for (size_t i = 0; i < count; ++i) { output[i] = double(input[i]); }
}

//----------------------------------------------------------------------------------------------------------------------
void doubleToFloat(float* output, const double* const input, size_t count)
{
    for (size_t i = 0; i < count; ++i) { output[i] = float(input[i]); }
}
...
void MeshImportContext::gatherFaceConnectsAndVertices()
{
    ...
    } else if (mesh.GetNormalsAttr().HasAuthoredValueOpinion()) {
    ...
    memcpy(
        &counts[0],
        (const int32_t*)faceVertexCounts.cdata(),
        sizeof(int32_t) * faceVertexCounts.size());
    memcpy(
        &connects[0],
        (const int32_t*)faceVertexIndices.cdata(),
        sizeof(int32_t) * faceVertexIndices.size());
    ...
    if (hasNormalsOpinion) {
        ...
        TfToken orientation;
        bool    leftHanded
            = (mesh.GetOrientationAttr().Get(&orientation, m_timeCode)
        ...
        if (leftHanded) {
            ...
            for (size_t j = 0; j < numPoints; ++j) { pnorm[j] = GfGetNormalized(pnorm[j]); }
...
void generateIncrementingIndices(MIntArray& indices, const size_t count)
{
    ...
    // trim a couple from the end of the array (I'm assuming maya isn't stupid enough to reallocate
    // upon shrinking by a small amount)
...
void MeshImportContext::applyHoleFaces()
{
    ...
    if (holeIndices.size()) {
        ...
        AL_MAYA_CHECK_ERROR2(
            fnMesh.setInvisibleFaces(mayaHoleIndices), "Unable to set invisible faces");
    }
}
...
void unzipUVs(const float* const uv, float* const u, float* const v, const size_t count)
{
    ...
    switch (count & 3) {
    case 3: u[i + 2] = uv[j + 4]; v[i + 2] = uv[j + 5];
    case 2: u[i + 1] = uv[j + 2]; v[i + 1] = uv[j + 3];
    case 1: u[i] = uv[j]; v[i] = uv[j + 1];
    default: break;
    }
...
bool MeshImportContext::applyVertexNormals()
{
    ...
    auto setUnlockedVertexNormals = [&](MVectorArray& normals) -> bool {
        ...
        for (uint32_t i = 0, n = fnMesh.numVertices(); i < n; ++i) { vertexList[i] = i; }
...
bool MeshImportContext::applyVertexCreases()
{
    ...
    if (cornerIndices.IsAuthored() && cornerIndices.HasValue() && cornerSharpness.IsAuthored()
        && cornerSharpness.HasValue()) {
        ...
        if (!fnMesh.setCreaseVertices(vertexIds, creaseData)) {
            std::cerr << "Unable to set crease vertices on mesh " << fnMesh.name().asChar()
                      << std::endl;
        }
        return true;
    }
...
bool MeshImportContext::applyEdgeCreases()
{
    ...
    if (creaseIndices.IsAuthored() && creaseIndices.HasValue() && creaseLengths.IsAuthored()
        && creaseLengths.HasValue() && creaseSharpness.IsAuthored() && creaseSharpness.HasValue()) {
        ...
        if (!fnMesh.setCreaseEdges(creaseEdgeIds, creaseValues)) {
            std::cerr << "Unable to set crease edges on mesh " << fnMesh.name().asChar()
                      << std::endl;
        }
        return true;
    }
            ...
            if (status) {
                ...
                if (status) {
                    ...
                    if (vtValue.IsHolding<VtArray<GfVec3f>>()) {
                        // If we can find the special displayColorToken used by USD, let's check for
                        // the optional matching displayOpacityToken too
                        ...
                        if (name == displayColorToken) {
                            ...
                            if (prim.HasAttribute(primvarDisplayOpacityToken)) {
                                UsdAttribute usdAttr
                                    = prim.GetAttribute(primvarDisplayOpacityToken);
                    ...
                    if (fnMesh.setUVs(u, v, uv_set)) {
                        if (interpolation == UsdGeomTokens->faceVarying) {
                            ...
                            for (uint32_t i = 0, j = 0; i < counts.length(); ++i) {
                                for (int k = 0; k < counts[i]; ++k) { mayaIndices[j++] = i; }
                            }
                            MStatus s = fnMesh.assignUVs(counts, mayaIndices, uv_set);
                            if (!s) {
                                TF_DEBUG(MAYAUSDUTILS_INFO)
...
void MeshExportContext::copyFaceConnectsAndPolyCounts()
{
    if ((diffMesh & kFaceVertexCounts) && faceCounts.length()) {
        ...
        memcpy(
            (int32_t*)faceVertexCounts.data(),
            &faceCounts[0],
            sizeof(uint32_t) * faceCounts.length());
    ...
    if ((diffMesh & kFaceVertexIndices) && faceConnects.length()) {
        ...
        memcpy(
            (int32_t*)faceVertexIndices.data(),
            &faceConnects[0],
            sizeof(uint32_t) * faceConnects.length());
...
void zipUVs(const float* u, const float* v, float* uv, const size_t count)
{
    ...
    switch (count & 3) {
    case 3: uv[4] = u[uvCount8 + 2]; uv[5] = v[uvCount8 + 2];
    case 2: uv[2] = u[uvCount8 + 1]; uv[3] = v[uvCount8 + 1];
    case 1: uv[0] = u[uvCount8 + 0]; uv[1] = v[uvCount8 + 0];
    default: break;
    }
    ...
    switch (count & 3) {
    case 3: uv[4] = u[uvCount4 + 2]; uv[5] = v[uvCount4 + 2];
    case 2: uv[2] = u[uvCount4 + 1]; uv[3] = v[uvCount4 + 1];
    case 1: uv[0] = u[uvCount4 + 0]; uv[1] = v[uvCount4 + 0];
    default: break;
    }
...
void MeshExportContext::copyUvSetData()
{
        ...
        if (fnMesh.getAssignedUVs(uvCounts, uvIds, &uvSetNames[i])) {
            ...
            if (!isUvSetDataSparse(ptr, uvCounts.length())) {
                if (fnMesh.getUVs(uValues, vValues, &uvSetNames[i])) {
                    ...
                    case kBasic:
                        interpolation
                            = guessUVInterpolationType(uValues, vValues, uvIds, faceConnects);
                        break;
                    case kMedium:
                        interpolation = guessUVInterpolationTypeExtended(
                            uValues, vValues, uvIds, faceConnects, uvCounts);
                        break;
                    case kFull:
                        interpolation = guessUVInterpolationTypeExtensive(
                            uValues, vValues, uvIds, faceConnects, uvCounts, indicesToExtract);
                        break;
                    }
                    ...
                    if (interpolation == UsdGeomTokens->constant) {
                        ...
                        for (uint32_t j = 0; j < nfaces; ++j) {
                            fnMesh.getPolygonUV(
                                j, 0, uvValues[j][0], uvValues[j][1], &uvSetNames[i]);
                        }
                        if (uvSetNames[i] == "map1") {
                            uvSetNames[i] = "st";
                        }
    ...
    for (uint32_t i = 0; i < diff_report.size(); i++) {
        ...
        } else if (diff_report[i].vertexInterpolation()) {
            const uint32_t npoints = fnMesh.numVertices();
            uvValues.resize(npoints);
        ...
        } else if (diff_report[i].uniformInterpolation()) {
            const uint32_t nfaces = fnMesh.numPolygons();
            uvValues.resize(nfaces);
            for (uint32_t j = 0; j < nfaces; ++j) {
                fnMesh.getPolygonUV(
                    j, 0, uvValues[j][0], uvValues[j][1], &diff_report[i].setName());
            }
            uvSet.Set(uvValues, m_timeCode);
            uvSet.SetInterpolation(UsdGeomTokens->uniform);
        } else if (diff_report[i].faceVaryingInterpolation()) {
            // Initialize the VtArray to the max possible size (facevarying)
...
#undef extract_ptr
    ...
    case 3: output[4] = u[indices[i + 2]]; output[5] = v[indices[i + 2]];
    case 2: output[2] = u[indices[i + 1]]; output[3] = v[indices[i + 1]];
    case 1: output[0] = u[indices[i]]; output[1] = v[indices[i]];
    default: break;
    }
...
void MeshExportContext::copyColourSetData()
{
    ...
    for (uint32_t i = 0; i < colourSetNames.length(); i++) {
        ...
        if (colourSetNames[i] == displayColorToken.GetText()) {
            ...
            if (interpolation == UsdGeomTokens->constant) {
                ...
                if (coloursLength) {
                    colourValues[0]
                        = GfVec4f(colours[0].r, colours[0].g, colours[0].b, colours[0].a);
                }
            } else {
                if (indicesToExtract.empty()) {
                    colourValues.resize(coloursLength);
    ...
    for (uint32_t i = 0; i < diff_report.size(); i++) {
        ...
        // if outputting as a vec3 (or we're writing to the displayColor GPrim schema attribute)
        if (MFnMesh::kRGB == representation
            || diff_report[i].setName() == displayColorToken.GetText()) {
            VtArray<GfVec3f> colourValues;
    ...
    if (diffMesh & (kCornerSharpness | kCornerIndices)) {
        ...
        if (status && creaseData.length() && vertIds.length()) {
            if (diffMesh & kCornerSharpness) {
                ...
                AL::usdmaya::utils::doubleToFloat(
                    subdCornerSharpnesses.data(), &creaseData[0], creaseData.length());
    ...
    if (diffMesh & (kCreaseWeights | kCreaseIndices | kCreaseLengths)) {
        ...
        if (status && edgeIds.length() && creaseData.length()) {
            ...
            if (diffMesh & kCreaseWeights) {
                ...
                AL::usdmaya::utils::doubleToFloat(
                    usdCreaseValues.data(), (double*)&creaseData[0], creaseData.length());
            ...
            // I'm not doing that at all (to be honest their approach looks to be questionable as to
            // whether it would actually work all that well, if at all).
...
//----------------------------------------------------------------------------------------------------------------------
// Loops through each Colour Set in the mesh writing out a set of non-indexed Colour Values in RGBA
// format, Renames Mayacolours sets, prefixing with "alusd_colour_" Writes out per-Face values only
// @todo: needs refactoring to handle face/vert/faceVarying correctly, allow separate RGB/A to be
// written etc.
void MeshExportContext::copyAnimalFaceColours()
{
    ...
    if (status == MS::kSuccess && colourSetNames.length()) {
        ...
        for (uint32_t i = 0; i < colourSetNames.length(); ++i) {
            ...
            UsdAttribute colour_set
                = mesh.GetPrim().CreateAttribute(TfToken(name), SdfValueTypeNames->Float4Array);
        ...
        if (UsdAttribute pointsAttr = mesh.GetPointsAttr()) {
            ...
            if (status) {
                ...
                MGlobal::displayError(
                    MString("Unable to access mesh vertices on mesh: ") + fnMesh.fullPathName());
            }
        }
    }
}
            ...
            if (status && numNormals) {
                ...
                if (MayaUsdUtils::vec3AreAllTheSame(normalsData, numNormals)) {
                    ...
                    if (MayaUsdUtils::compareArray(
                        ...
                        memcpy(
                            (GfVec3f*)normals.data(), normalsData, sizeof(float) * 3 * numNormals);
                        normalsAttr.Set(normals, time);
                    } else {
                        ...
                        if (isPerVertex) {
                            VtArray<GfVec3f> normals(numNormals);
                            memcpy(
                                (GfVec3f*)normals.data(),
                                normalsData,
                                sizeof(float) * 3 * numNormals);
                            for (auto& c : missing) {
                            ...
                            if (copyAsPrimvar) {
                                ...
                                memcpy(
                                    normalIds.data(),
                                    &normalIndices[0],
                                    sizeof(int) * normalIndices.length());
                    ...
                    if (isOrdered) {
                        ...
                        for (uint32_t i = 0, n = normalIndices.length(); i < n; ++i) {
                            const uint32_t index = 3 * normalIndices[i];
                            normals[i] = GfVec3f(
                                normalsData[index], normalsData[index + 1], normalsData[index + 2]);
                        }
                        normalsAttr.Set(normals, time);
                    }
                }
                ...
                MGlobal::displayError(
                    MString("Unable to access mesh normals on mesh: ") + fnMesh.fullPathName());
            }
        }
    }
}
