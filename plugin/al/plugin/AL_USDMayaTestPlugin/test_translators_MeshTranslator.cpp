...
TEST(translators_MeshTranslator, convert3DArrayTo4DArray)
{
    ...
    input.resize(39 * 3);
    output.resize(39 * 4);
    for (uint32_t i = 0; i < 39 * 3; ++i) { input[i] = float(i); }
...
TEST(translators_MeshTranslator, generateIncrementingIndices)
{
    ...
    for (int32_t i = 0; i < 39; ++i) { EXPECT_EQ(indices[i], i); }
}
...
TEST(translators_MeshTranslator, vertexColourExport)
{
...

    {
        ...
        MColorArray colours;
        colours.setLength(8);
        for (int i = 0; i < 8; ++i) { colours[i] = MColor(0.3f * i, 0.4f, 0.5f, 1.0f); }
        fn.setColors(colours, &name);
        fn.assignColors(indices, &name);
    }
...
TEST(translators_MeshTranslator, uniformColourExport)
{
...

    {
        ...
        MColorArray colours;
        colours.setLength(6);
        for (int i = 0; i < 6; i++) { colours[i] = MColor(0.3f * float(i), 0.4f, 0.5f, 1.0f); }
        indices.setLength(24);
        for (int i = 0; i < 24; i++) { indices[i] = i / 4; }
        fn.setColors(colours, &name);
        fn.assignColors(indices, &name);
    }
...
TEST(translators_MeshTranslator, reverseNormalsFlag)
{
...

    {
        ...
        for (size_t i = 0; i < 24; ++i) { EXPECT_EQ(expectedNormals[i], normals[i]); }
