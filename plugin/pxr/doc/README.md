...
`-rdc` | `-renderContext` | string | `preview` | Selects a render context for export. The default value `preview` will export to a UsdPreviewSurface shading netword. A plugin mechanism allows more render contexts to be registered. Use the `mayaUSDListShadingModes` command to explore the possible options.
