...
{
  AL::maya::MenuBuilder::clearRootMenus();
  EXPECT_TRUE(AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI", "sphere", "sphereOB"));
  EXPECT_EQ(size_t(1), AL::maya::MenuBuilder::rootMenus().size());

  const AL::maya::MenuBuilder::Menu& menu0 = *AL::maya::MenuBuilder::rootMenus().begin();
  EXPECT_EQ(std::string("FOO"), menu0.name());
  EXPECT_EQ(size_t(1), menu0.childMenus().size());
  EXPECT_EQ(size_t(0), menu0.menuItems().size());

  const AL::maya::MenuBuilder::Menu& menu1 = *menu0.childMenus().begin();
  EXPECT_EQ(std::string("BAR"), menu1.name());
  EXPECT_EQ(size_t(1), menu1.childMenus().size());
  EXPECT_EQ(size_t(0), menu1.menuItems().size());

  const AL::maya::MenuBuilder::Menu& menu2 = *menu1.childMenus().begin();
  EXPECT_EQ(std::string("MOO"), menu2.name());
  EXPECT_EQ(size_t(0), menu2.childMenus().size());
  EXPECT_EQ(size_t(1), menu2.menuItems().size());

  const AL::maya::MenuBuilder::MenuItem& item = menu2.menuItems()[0];
...
TEST(maya_MenuBuilder, sharedPath)
{
  AL::maya::MenuBuilder::clearRootMenus();
  EXPECT_TRUE(AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI", "sphere", "sphereOB"));
  EXPECT_TRUE(AL::maya::MenuBuilder::addEntry("FOO/BAR/OINK/HI", "cube", "cubeOB"));
  EXPECT_EQ(size_t(1), AL::maya::MenuBuilder::rootMenus().size());
...

  {
    ...
    ++it;
    const AL::maya::MenuBuilder::Menu& menu2 = *it;
...
TEST(maya_MenuBuilder, sharedPath2)
{
  AL::maya::MenuBuilder::clearRootMenus();
  EXPECT_TRUE(AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI", "sphere", "sphereOB"));
  EXPECT_TRUE(AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/BYE", "cube", "cubeOB"));
  EXPECT_EQ(size_t(1), AL::maya::MenuBuilder::rootMenus().size());
  ...
  const AL::maya::MenuBuilder::MenuItem& item1 = menu2.menuItems()[0];
  ...
  const AL::maya::MenuBuilder::MenuItem& item2 = menu2.menuItems()[1];
...
TEST(maya_MenuBuilder, duplicatePath)
{
  AL::maya::MenuBuilder::clearRootMenus();
  EXPECT_TRUE(AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI", "sphere", "sphereOB"));
  EXPECT_FALSE(AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI", "cube", "cubeOB"));
  EXPECT_EQ(size_t(1), AL::maya::MenuBuilder::rootMenus().size());
...
{
  AL::maya::MenuBuilder::clearRootMenus();
  EXPECT_TRUE(0 != AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI1", "Checky", true, false));
  EXPECT_TRUE(0 != AL::maya::MenuBuilder::addEntry("FOO/BAR/MOO/HI2", "McCheckFace", true, true));
  EXPECT_EQ(size_t(1), AL::maya::MenuBuilder::rootMenus().size());
